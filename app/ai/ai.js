...
function scoreDevelopmentAction(state, player, action) {
    ...
    if (action.kind === "extract" || action.kind === "harvestBiomass") {
        ...
        if (strategy === "fossilTempo" && resource === "fossilFuel" && state.generation >= 4)
            factors.push(factor("transition", "Fuel bridge ending", -(state.generation - 2) * 3, "Late fossil extraction competes with building replacement capacity."));
    }
    ...
    if (action.kind === "build") {
        ...
        if (strategy === "fossilTempo" && state.generation >= 3) {
            const transitionStrength = state.generation - 2;
            if (technology.pathway === "fossil")
                factors.push(factor("transition", "Fuel bridge ending", -transitionStrength * 4, "Additional fossil capacity becomes less valuable as the bridge period closes."));
                ...
                factors.push(factor("transition", "Replacement pathway", (alreadyRepresented ? 2 : 6) * transitionStrength, "The Fuel Bridge strategy must replace finite generation before late Reliability targets rise."));
            }
        }
...
export function considerAiTrade(state, proposer, utilityThreshold = 0.35) {
    if (state.phase !== "generation.development" || !state.config.trade.directEnabled || proposer.completedTrades >= state.config.trade.directTradesPerGeneration)
        return null;
