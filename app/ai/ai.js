...
function estimateBuild(state, player, technology) {
    ...
    const deferredBlueprint = player.prepared.pathwayId && !player.prepared.pathwayUsed && player.prepared.foundingProjectDeferred && technology.pathway === player.prepared.pathwayId;
    const fossilBlueprint = technology.id === "combinedCycle" && player.prepared.fossilBlueprintAvailable;
...
function marginalTechnologyCapacity(state, player, technology) {
    ...
    if (technology.stage === "transport") {
        ...
        const capacityGain = Math.max(0, Math.min(technology.capacity, 6 + conditionNeed - existing));
    ...
    if (technology.storage?.type === "battery") {
        ...
        const capacityGain = Math.max(0, Math.min(technology.storage.capacity, desired - existing));
    ...
    if (technology.pathway === "hydro" && technology.stage === "transformation") {
        ...
        const capacityGain = Math.max(0, Math.min(technology.capacity, continent.opportunities.hydro - existing));
        return Math.max(capacityGain, technology.special === "advancedHydroFlexibility" ? 1 : 0);
    }
    ...
    if (technology.pathway === "solar" || technology.pathway === "wind") {
        ...
        const capacityGain = Math.max(0, Math.min(technology.capacity, continent.opportunities[technology.pathway] - existing));
...
function foundingProjectCost(state, player) {
    ...
    return getTechnology(state, pathway.foundingTechnologyId).cost;
}
function summitResourceValue(state, player, resource) {
    const cost = foundingProjectCost(state, player);
    ...
    if (projectDeficit > 0)
        value += 8;
    if (resource === "fossilFuel" && player.prepared.pathwayId === "fossil")
        value += 2;
    if (resource === "biomass" && player.prepared.pathwayId === "biomass")
        value += 2;
    if (held <= required)
        value += 2;
    if (held >= required + 2)
...
export function chooseAiSummitOffer(state, player) {
    ...
    for (const recipient of Object.values(state.players)) {
        ...
        for (const requested of resourceTypes) {
            if (recipient.resources[requested].warehouse <= 0)
                continue;
            const requestedValue = summitResourceValue(state, player, requested);
            for (const offered of resourceTypes) {
                if (offered === requested || player.resources[offered].warehouse <= 0)
                    continue;
                const offeredCost = summitResourceValue(state, player, offered);
                const recipientGain = summitResourceValue(state, recipient, offered);
                const recipientCost = summitResourceValue(state, recipient, requested);
                const score = (requestedValue - offeredCost) + (recipientGain - recipientCost) * 0.7;
                if (score > 1.5 && (!best || score > best.score))
                    best = { score, recipientId: recipient.id, proposerGives: { [offered]: 1 }, recipientGives: { [requested]: 1 } };
            }
        }
    }
    return best;
