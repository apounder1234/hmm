...
import { conditionApplies, hasRelevantSystem, emptyEnergy, fuelPlantMaximumOutput, fossilChainSnapshot, getTechnology, hasTechnology, log, pathways, resourceTypes, totalEnergy, getContinentGenerationModifiers, getContinentProfile, getEffectiveUpgradeCost, getKnowledgeRequirement, getPathwayAffinity, getTransmissionLevel, gatherAmount } from "./rules.js";
// -----------------------------------------------------------------------------
...
function capture(state, player, pathway) {
    ...
    let output = state.config.weather[pathway][state.weather.current][Math.min(5, capacity)] ?? 0;
    const condition = conditionApplies(state, player);
...
function hydroRequest(state, player) {
    const reservoirs = player.installed.filter(instance => operational(state, instance.builtGeneration) && getTechnology(state, instance.technologyId).storage?.type === "reservoir");
    const turbine = player.installed
        .filter(instance => operational(state, instance.builtGeneration))
        .map(instance => getTechnology(state, instance.technologyId))
        .filter(technology => technology.pathway === "hydro" && technology.stage === "transformation")
        .reduce((sum, technology) => sum + technology.capacity, 0);
    let inflow = state.config.weather.hydro[state.weather.current][Math.min(5, turbine)] ?? 0;
    const condition = conditionApplies(state, player);
    ...
    return Math.min(turbine, stored + Math.min(Math.max(0, inflow), room));
}
