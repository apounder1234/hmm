...
import { effectivePathwayOpportunity, fuelPlantMaximumOutput, getTechnology, hasTechnology, pathways, totalEnergy } from "../engine/helpers.js";
...
export function systemSnapshot(state, playerId, options = {}) {
    ...
    const lightCeiling = Math.min(state.config.demand.maximumLight, availableNow, grid, lightingMaximum);
    return {
        target,
...
export function technologyImpactPreview(state, playerId, technologyOrId) {
    ...
    const metrics = [];
    const push = item => { if (item) metrics.push(item); };
    let now = "";
    let future = "";
...
export function conditionImpactPreview(state, playerId, condition) {
    ...
    switch (effect.kind) {
        ...
        case "firstBuildConstructionDelta":
            ...
            impact = `The first technology you build this Generation costs ${effect.amount} extra General Material.`;
...
export function systemGuidance(state, playerId) {
    ...
    if (best) {
        ...
        const links = knowledgeLinkOptions(state, player, best.technology);
        const action = legal.legal
            ? `Build ${best.technology.name}.`
            : links.length
                ? `Use a Knowledge Link to build ${best.technology.name}.`
...
export function knowledgeLinkOptions(state, player, technology) {
    if (!state.config.trade.knowledgeLinkEnabled || player.knowledgeLinkUsed || player.actionsRemaining <= 0)
        return [];
    const options = [];
    for (const lender of Object.values(state.players)) {
        if (lender.id === player.id || lender.knowledgeLinkUsed || lender.knowledge < technology.knowledgeRequired)
            continue;
        for (const paymentResource of resourceOrder) {
            try {
                const draft = structuredClone(state);
                applyCommand(draft, {
                ...
                options.push({ lenderId: lender.id, lenderName: lender.name, paymentResource });
            }
            catch {
                // Only authoritative engine-valid Knowledge Links are exposed.
            }
        }
    }
    return options;
