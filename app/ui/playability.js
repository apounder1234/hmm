...
import { getContinentGenerationModifiers, getEffectiveUpgradeCost, getKnowledgeRequirement, getTransmissionLevel } from "../engine/continentRules.js";
...
export function knowledgeLinkOptions(state, player, technology) {
    if (!state.config.trade.knowledgeLinkEnabled || player.knowledgeLinkUsed || player.actionsRemaining <= 0)
        return [];
    const options = [];
    const knowledgeRequired = getKnowledgeRequirement(state, player, technology);
    for (const lender of Object.values(state.players)) {
        if (lender.id === player.id || lender.knowledgeLinkUsed || lender.knowledge < knowledgeRequired)
            continue;
