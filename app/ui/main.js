...
const resourceLabels = { fossilFuel: "Fossil Fuel", biomass: "Biomass", constructionMaterials: "General Materials", criticalMaterials: "Critical Minerals" };
...
function localConditionExplanation(game, player, condition) {
    ...
    const descriptions = {
        ...
        firstBuildConstructionDelta: `Your first Build this Generation costs ${effect.amount} additional General Material.`,
...
function technologyExplanation(game, player, tech) {
    ...
    const details = [
        ...
        `Cost: ${tech.cost.constructionMaterials} General Materials and ${tech.cost.criticalMaterials} Critical Minerals. Requires Knowledge ${tech.knowledgeRequired}.`,
    ...
    if (tech.knowledgeRationale)
        details.push(`Why Knowledge ${tech.knowledgeRequired}: ${tech.knowledgeRationale}`);
    if (tech.fuel)
    ...
    return { eyebrow: `Knowledge ${tech.knowledgeRequired} technology`, title: tech.name, summary: impact.headline, details };
}
...
function actionConfirmation(game, player, action) {
    ...
    if (action.kind === "research") {
        ...
        const payment = [`${cost.general} General Materials`, `${cost.critical} Critical Minerals`];
        return {
            title: "Learn and gain Knowledge",
            summary: `Increase Knowledge from ${player.knowledge} to ${cost.nextLevel}.`,
            details: [
                `Cost: ${payment.join(" + ")}.`,
                cost.usesAppliedLearning ? "One Applied Learning token replaces 1 General Material." : "Knowledge becomes more expensive at higher levels.",
    ...
    if (action.kind === "build") {
        ...
        return {
            ...
            details: [
                `Actual cost now: ${cost.constructionMaterials} General Materials + ${cost.criticalMaterials} Critical Minerals.`,
                `Requires Knowledge ${tech.knowledgeRequired}; your effective Knowledge is ${cost.effectiveKnowledge}.`,
...
function aiStep(game) {
    ...
    if (game.phase === "setup.foundingProjects") {
        ...
        const cost = pathway.id === "fossil" ? pathway.foundingCost : getTechnology(game, pathway.foundingTechnologyId).cost;
        const complete = player.resources.constructionMaterials.warehouse >= cost.constructionMaterials && player.resources.criticalMaterials.warehouse >= cost.criticalMaterials;
...
function FoundingProjectPanel({ game, command }) {
    ...
    const pathway = game.config.preparedPathways.find(item => item.id === player.prepared.pathwayId);
    const project = pathway.id === "fossil" ? { name: pathway.foundingLabel, cost: pathway.foundingCost } : { name: getTechnology(game, pathway.foundingTechnologyId).name, cost: getTechnology(game, pathway.foundingTechnologyId).cost };
    const affordable = player.resources.constructionMaterials.warehouse >= project.cost.constructionMaterials && player.resources.criticalMaterials.warehouse >= project.cost.criticalMaterials;
    return panel(`Founding Project · ${player.name}`, h("div", { className: "founding-project" }, h("p", null, `Revealed plan: ${pathwayLabels[player.prepared.pathwayId]} + ${capabilityLabels[player.prepared.capabilityId]}.`), h("strong", null, project.name), h("p", null, `Cost: ${project.cost.constructionMaterials} General Materials + ${project.cost.criticalMaterials} Critical Minerals. No Generation 1 action is used.`), affordable ? button("Complete Founding Project", () => command({ type: "resolveFoundingProject", playerId: player.id, complete: true }), { kind: "primary large" }) : h("div", { className: "notice warning" }, "You cannot pay this project now. Defer it and keep a one-use pathway Blueprint for a later build."), button("Defer project", () => command({ type: "resolveFoundingProject", playerId: player.id, complete: false }), { kind: "ghost" })));
}
...
function PlayerBoard({ game, player, onInfo }) {
    ...
    return h("div", { className: "player-board" }, h("div", { className: "board-heading" }, h("div", null, h("p", { className: "eyebrow" }, "Player board"), h("h2", null, player.name)), h("div", { className: "badge-stack" }, h("button", { type: "button", className: "badge explain-badge", onClick: () => onInfo({ eyebrow: "Permanent capability", title: `Knowledge ${player.knowledge}`, summary: "Knowledge unlocks the 1–3–5 technology ladder and can earn resources through a Knowledge Link.", details: ["Learn uses one Development action and materials to permanently increase Knowledge by one, to a maximum of five.", nextCost ? `Next level currently costs ${nextCost.general} General Materials and ${nextCost.critical} Critical Minerals${nextCost.usesAppliedLearning ? ", after using one Applied Learning token" : ""}.` : "You have reached maximum Knowledge.", "Once per Generation, a player may borrow your Knowledge for one Build and pay you one resource. Both players then use their Knowledge Link for that Generation."] }) }, `Knowledge ${player.knowledge}`, h("i", { className: "micro-help" }, "?")), h("span", { className: `badge ${player.knowledgeLinkUsed ? "warning" : ""}` }, player.knowledgeLinkUsed ? "Link used" : "Knowledge Link ready"), player.appliedLearningTokens ? h("span", { className: "badge energy" }, `Applied Learning ${player.appliedLearningTokens}`) : null, condition ? h("button", { type: "button", className: "badge condition explain-badge", onClick: () => onInfo(localConditionExplanation(game, player, condition)) }, condition.name, h("i", { className: "micro-help" }, "?")) : null)), h(EnergyChain, { player, onInfo }), h("div", { className: "two-column" }, panel("Warehouse", h(Warehouse, { game, player, onInfo }), "nested"), panel("Installed technologies", h(TechnologyList, { game, player, onInfo }), "nested")), panel("Light and Reliability", h(LightTrack, { game, player, onInfo }), "nested"), h("div", { className: "score-row" }, stat("Total Light", player.cumulative.totalLight), stat("Reliability Points", `${player.cumulative.reliableGenerations}/${game.config.rules.reliabilityPointMaximum ?? 4}`), stat("Demand met", `${player.cumulative.demandMetGenerations ?? 0}/8`), stat("System Loss", totalLoss(player)), stat("Curtailment", player.cumulative.curtailment)));
}
...
function BuildShop({ game, player, guidedMode, onChoose, onKnowledgeLink, onInfo }) {
    ...
    return h("div", { className: "guided-subpanel" },
        ...
        h("div", { className: "clean-card-list" }, ...visible.map(tech => {
            ...
            const impact = technologyImpactPreview(game, player.id, tech);
            const canAffordGeneral = player.resources.constructionMaterials.warehouse >= effectiveCost.constructionMaterials;
            const canAffordCritical = player.resources.criticalMaterials.warehouse >= effectiveCost.criticalMaterials;
            const reachable = legality.legal || links.length > 0;
            const tierLabel = tech.knowledgeRequired === 1 ? "Basic" : tech.knowledgeRequired === 3 ? "Upgrade" : "Advanced";
            return h("article", { key: tech.id, className: `clean-choice-card technology-impact-card ${reachable ? "available" : "locked"}` },
                h("div", { className: "clean-choice-main" }, h("div", { className: "choice-icon" }, tech.stage === "storage" ? "▣" : tech.stage === "transport" ? "⚡" : tech.stage === "research" ? "✦" : selectedPath === "solar" ? "☀" : selectedPath === "wind" ? "≋" : selectedPath === "hydro" ? "💧" : selectedPath === "biomass" ? "🌿" : selectedPath === "fossil" ? "⛽" : "⚙"), h("div", null, h("strong", null, tech.name), h("small", null, `${tierLabel} · Knowledge ${tech.knowledgeRequired}`))),
                h("div", { className: "cost-chips" }, badge(`${effectiveCost.constructionMaterials} General`, canAffordGeneral ? "" : "warning"), badge(`${effectiveCost.criticalMaterials} Critical`, canAffordCritical ? "" : "warning"), badge(`K${tech.knowledgeRequired}`)),
                h("div", { className: "impact-summary" },
                    h("div", { className: "impact-heading" }, h("strong", null, impact.headline), badge(impact.timing, impact.timing === "Helps now" ? "reliable" : impact.timing === "Not useful here" ? "warning" : "energy")),
                    impact.metrics.length ? h("div", { className: "impact-metrics" }, ...impact.metrics.slice(0, 3).map(metric => h("span", { key: metric.label }, h("small", null, metric.label), h("b", null, `${metric.before}${metric.unit} → ${metric.after}${metric.unit}`)))) : null,
                    h("p", null, impact.now),
                    h("small", { className: "forecast-benefit" }, impact.future),
                    h("small", { className: "next-bottleneck" }, impact.prompt)),
                !legality.legal && links.length ? h("p", { className: "choice-benefit" }, `${links.length} Knowledge Link option${links.length === 1 ? "" : "s"}. The helper receives one resource.`) : !legality.legal ? h(LockedReason, { reason: legality.reason }) : null,
                h("div", { className: "choice-actions" }, infoButton(() => onInfo(technologyExplanation(game, player, tech)), `Explain ${tech.name}`), legality.legal ? button("Build", () => onChoose({ kind: "build", technologyId: tech.id }), { kind: "primary compact" }) : links.length ? button("Use Knowledge Link", () => setLinkRequest({ tech, value: `${links[0].lenderId}|${links[0].paymentResource}` }), { kind: "secondary compact" }) : button("Locked", () => {}, { kind: "ghost compact", disabled: true, title: legality.reason }))
            );
        }))
    );
}
...
function GatherPanel({ game, player, onChoose }) {
    const choices = [
        { action: { kind: "extract", resource: "constructionMaterials" }, icon: "▦", title: "General Materials", note: "Steel, cement, glass, timber, machinery and other construction inputs." },
...
function GameScreen({ game, setGame, onHome, onRules, onLoad }) {
    ...
    const market = game.innovationMarket.visible.length
        ? panel("Advanced technology market", h("div", { className: "market-list" }, ...game.innovationMarket.visible.map(id => {
            ...
            return h("button", { type: "button", key: id, className: "market-mini-card", onClick: () => setInfo(technologyExplanation(game, selected, tech)) }, h("strong", null, tech.name), h("small", null, `${tech.cost.constructionMaterials} General · ${tech.cost.criticalMaterials} Critical · Knowledge ${tech.knowledgeRequired}`));
        })), "nested")
