...
const resourceDescriptions = {
    fossilFuel: "Finite fuel used by Fossil plants. Extract it from the continent into your Warehouse before it can be consumed or traded.",
...
function resourceExplanation(player, key) {
    const account = player.resources[key];
    return {
        eyebrow: "Warehouse resource",
        title: resourceLabels[key],
        summary: resourceDescriptions[key],
        details: [`Warehouse: ${account.warehouse}/9.`, `Remaining in continent: ${account.currentContinent}.`, `Printed starting stock: ${account.printedStarting}. Only Warehouse resources may be spent, consumed or traded.`]
    };
}
...
function SetupScreen({ config, participants, setParticipants, seed, setSeed, debugMode, setDebugMode, onNewSeed, onStart, onBack }) {
...
function Warehouse({ player, onInfo }) {
    return h("div", { className: "warehouse-grid" }, ...resourceKeys.map(key => {
        const account = player.resources[key];
        return h("button", { type: "button", key, className: "resource-cell", onClick: () => onInfo(resourceExplanation(player, key)) }, h("span", null, resourceLabels[key], h("i", { className: "micro-help" }, "?")), h("strong", null, account.warehouse), h("small", null, `${account.currentContinent} in continent · printed ${account.printedStarting}`));
    }));
}
...
function PlayerBoard({ game, player, onInfo }) {
    const condition = conditionDefinition(game, player);
    return h("div", { className: "player-board" }, h("div", { className: "board-heading" }, h("div", null, h("p", { className: "eyebrow" }, "Player board"), h("h2", null, player.name)), h("div", { className: "badge-stack" }, h("button", { type: "button", className: "badge explain-badge", onClick: () => onInfo({ eyebrow: "Permanent capability", title: `Knowledge ${player.knowledge}`, summary: "Knowledge unlocks technologies and is never spent when you build.", details: ["Research uses one Development action and permanently increases Knowledge by one, to a maximum of five.", "Technical Assistance and some cards can add temporary Knowledge for a construction."] }) }, `Knowledge ${player.knowledge}`, h("i", { className: "micro-help" }, "?")), condition ? h("button", { type: "button", className: "badge condition explain-badge", onClick: () => onInfo(localConditionExplanation(game, player, condition)) }, condition.name, h("i", { className: "micro-help" }, "?")) : null)), h(EnergyChain, { player, onInfo }), h("div", { className: "two-column" }, panel("Warehouse", h(Warehouse, { player, onInfo }), "nested"), panel("Installed technologies", h(TechnologyList, { game, player, onInfo }), "nested")), panel("Light and Reliability", h(LightTrack, { game, player, onInfo }), "nested"), h("div", { className: "score-row" }, stat("Total Light", player.cumulative.totalLight), stat("Reliable Generations", player.cumulative.reliableGenerations), stat("System Loss", totalLoss(player)), stat("Curtailment", player.cumulative.curtailment)));
}
function BuildShop({ game, onAction, onInfo }) {
    ...
    return h("div", { className: "build-shop" }, ...available.map(tech => h("article", { key: tech.id, className: "shop-card" }, h("div", { className: "shop-card-heading" }, h("strong", null, tech.name), infoButton(() => onInfo(technologyExplanation(tech)), `Explain ${tech.name}`)), h("small", null, `${tech.tier} · ${titleCase(tech.stage)}`), h("span", null, `${tech.cost.constructionMaterials} Construction · ${tech.cost.criticalMaterials} Critical · K${tech.knowledgeRequired}`), h("em", null, `Capacity ${tech.capacity}`), button("Build", () => onAction({ kind: "build", technologyId: tech.id }), { kind: "shop-build" }))));
}
...
function GameScreen({ game, setGame, onHome, onRules, onLoad }) {
    ...
    if (game.phase.startsWith("setup."))
        ...
        decision = active?.controller.kind === "human" ? h(DevelopmentControls, { game, player: active, onAction: act, onUndo: () => command({ type: "undo" }), onReset: () => command({ type: "resetGeneration" }), onTrade: trade, tradeMessage, onInfo: setInfo }) : panel("AI planning", h("p", null, "AI players are resolving their Development actions."));
    }
    ...
    const body = game.phase === "game.complete"
        ? decision
        : h("main", { className: "game-main" }, h("div", { className: "game-left" }, h(WorldArea, { game, selectedId, setSelectedId }), h(PlayerBoard, { game, player: selected, onInfo: setInfo })), h("aside", { className: "game-right" }, decision, market, game.debugMode ? h(AiDebugPanel, { game }) : null, events));
    return h("div", { className: "game-shell" }, h(GameHeader, { game, selectedPlayer: selected, onInfo: setInfo, onHome, onRules, onSave: save, onLoad }), notice ? h("div", { className: "notice error" }, notice, button("Dismiss", () => setNotice(""), { kind: "ghost" })) : null, body, info ? h(InfoModal, { info, onClose: () => setInfo(null) }) : null);
}
...
class AppErrorBoundary extends React.Component {
    ...
    render() {
        ...
        return h("main", { className: "crash-screen" }, h("p", { className: "eyebrow" }, "The interface hit an unexpected state"), h("h1", null, "SUNPATHS did not go blank"), h("p", null, "The error is shown here so it can be reported and repaired. Reloading returns to the start screen."), h("pre", null, String(this.state.error?.stack || this.state.error?.message || this.state.error)), h("div", { className: "form-row" }, button("Reload game", () => window.location.reload(), { kind: "primary" })));
    }
}
function App() {
    ...
    return h(React.Fragment, null, fatal ? h("div", { className: "fatal-banner" }, fatal, button("Dismiss", () => setFatal(""), { kind: "ghost" })) : null, screen === "start" ? h(StartScreen, { onNew: () => { setParticipants(makeParticipants(config)); setSeed(newRandomSeed()); setScreen("setup"); }, onLoad: loadFile, onRules: () => setScreen("rules"), onSimulation: () => setScreen("simulation") }) : null, screen === "setup" ? h(SetupScreen, { config, participants, setParticipants, seed, setSeed, debugMode, setDebugMode, onNewSeed: () => setSeed(newRandomSeed()), onStart: startGame, onBack: () => setScreen("start") }) : null, screen === "simulation" ? h(SimulationLab, { config, onBack: () => setScreen("start") }) : null, screen === "rules" ? h(RulesScreen, { config, setConfig, onBack: () => setScreen(game ? "game" : "start") }) : null, screen === "game" && game ? h(GameScreen, { game, setGame, onHome: home, onRules: () => setScreen("rules"), onLoad: loadFile }) : null);
}
