...
import { buildLegality, developmentActionLegality, importLegality, previewDispatch } from "./playability.js";
...
function BuildShop({ game, player, guidedMode, onChoose, onInfo }) {
    ...
    return h("div", { className: "guided-subpanel" }, h("div", { className: "pathway-tabs", role: "tablist", "aria-label": "Energy pathway" }, ...pathOptions.map(path => h("button", { key: path, type: "button", className: `pathway-tab ${selectedPath === path ? "selected" : ""}`, onClick: () => setSelectedPath(path) }, path === "system" ? "⚙ System" : `${path === "solar" ? "☀" : path === "wind" ? "≋" : path === "hydro" ? "💧" : path === "biomass" ? "🌿" : "⛽"} ${pathwayLabels[path]}`))), filtered.length === 0 ? h("p", { className: "empty-guidance" }, "No technology from this pathway is available in the market right now.") : null, h("div", { className: "clean-card-list" }, ...visible.map(tech => {
        ...
        const canAffordMaterials = player.resources.constructionMaterials.warehouse >= tech.cost.constructionMaterials && player.resources.criticalMaterials.warehouse >= tech.cost.criticalMaterials;
        return h("article", { key: tech.id, className: `clean-choice-card ${legality.legal ? "available" : "locked"}` }, h("div", { className: "clean-choice-main" }, h("div", { className: "choice-icon" }, tech.stage === "storage" ? "▣" : tech.stage === "transport" ? "⚡" : tech.stage === "research" ? "✦" : selectedPath === "solar" ? "☀" : selectedPath === "wind" ? "≋" : selectedPath === "hydro" ? "💧" : selectedPath === "biomass" ? "🌿" : selectedPath === "fossil" ? "⛽" : "⚙"), h("div", null, h("strong", null, tech.name), h("small", null, `${titleCase(tech.stage)} · Capacity ${tech.capacity}`))), h("div", { className: "cost-chips" }, badge(`${tech.cost.constructionMaterials} Building`, canAffordMaterials ? "" : "warning"), badge(`${tech.cost.criticalMaterials} Special`, canAffordMaterials ? "" : "warning"), badge(`Knowledge ${tech.knowledgeRequired}`)), legality.legal ? h("p", { className: "choice-benefit" }, tech.storage ? `Stores up to ${tech.storage.capacity} Energy.` : tech.fuel ? `Uses 1 ${resourceLabels[tech.fuel.resource]} to make up to ${tech.maximumOutput} Energy.` : `Can use up to ${tech.capacity} of this pathway's natural opportunity.`) : h(LockedReason, { reason: legality.reason }), h("div", { className: "choice-actions" }, infoButton(() => onInfo(technologyExplanation(tech)), `Explain ${tech.name}`), button(legality.legal ? "Choose" : "Locked", () => onChoose({ kind: "build", technologyId: tech.id }), { kind: legality.legal ? "primary compact" : "ghost compact", disabled: !legality.legal, title: legality.reason })));
    })));
}
...
function TradePanel({ game, player, onTrade, message }) {
    ...
    const canPropose = player.completedTrades < game.config.trade.directTradesPerGeneration && ownOffer >= 1 && recipientHas >= 1;
    const reason = player.completedTrades >= game.config.trade.directTradesPerGeneration ? "You already completed your direct trade this Generation." : ownOffer < 1 ? `You have no ${resourceLabels[offer]} to offer.` : recipientHas < 1 ? `${recipient?.name || "That player"} has no ${resourceLabels[request]} to give.` : "Available";
...
function App() {
    ...
    return h(React.Fragment, null, fatal ? h("div", { className: "fatal-banner" }, fatal, button("Dismiss", () => setFatal(""), { kind: "ghost" })) : null, screen === "start" ? h(StartScreen, { onNew: () => { setParticipants(makeParticipants(config)); setSeed(newRandomSeed()); setScreen("setup"); }, onLoad: loadFile, onRules: () => setScreen("rules"), onSimulation: () => setScreen("simulation") }) : null, screen === "setup" ? h(SetupScreen, { config, participants, setParticipants, seed, setSeed, debugMode, setDebugMode, playMode, setPlayMode, onNewSeed: () => setSeed(newRandomSeed()), onStart: startGame, onBack: () => setScreen("start") }) : null, screen === "simulation" ? h(SimulationLab, { config, onBack: () => setScreen("start") }) : null, screen === "rules" ? h(RulesScreen, { config, setConfig, onBack: () => setScreen(game ? "game" : "start") }) : null, screen === "game" && game ? h(GameScreen, { game, setGame, onHome: home, onRules: () => setScreen("rules"), onLoad: loadFile }) : null);
}
