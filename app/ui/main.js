...
import { buildLegality, developmentActionLegality, effectiveBuildCost, importLegality, knowledgeLinkOptions, previewDispatch } from "./playability.js";
...
function technologyExplanation(tech) {
    ...
    if (tech.prerequisiteTechnologyId)
        details.push(`Upgrade requirement: build ${titleCase(tech.prerequisiteTechnologyId)} first.`);
    ...
    if (tech.fuel) {
        ...
        if (tech.pathway === "fossil")
            details.push("Fossil Opportunity falls as the finite stock is consumed. Even Knowledge-5 fossil technology cannot remove depletion.");
    }
    if (tech.storage)
    ...
    return { eyebrow: `Knowledge ${tech.knowledgeRequired} technology`, title: tech.name, summary: `${tech.tier === "basic" ? "Basic" : tech.tier === "intermediate" ? "Upgraded" : "Advanced"} technology.`, details };
}
...
function actionConfirmation(game, player, action) {
    ...
    if (action.kind === "build") {
        ...
        return { title: `Build ${tech.name}?`, summary: `Add this Knowledge-${tech.knowledgeRequired} ${titleCase(tech.stage)} technology to your energy pathway.`, details: [`Printed cost: ${tech.cost.constructionMaterials} General Materials + ${tech.cost.criticalMaterials} Critical Minerals.`, `Requires your own Knowledge ${tech.knowledgeRequired}; you have ${player.knowledge + player.temporaryKnowledge}.`, tech.prerequisiteTechnologyId ? `Requires the previous technology in this pathway first.` : "This is the basic technology in its pathway.", `Capacity: ${tech.capacity}. It may operate this Generation under the current prototype rule.`], confirmLabel: "Build it" };
    }
    if (action.kind === "publicImport") {
...
function BuildShop({ game, player, guidedMode, onChoose, onKnowledgeLink, onInfo }) {
    ...
    return h("div", { className: "guided-subpanel" }, h("div", { className: "pathway-tabs", role: "tablist", "aria-label": "Energy pathway" }, ...pathOptions.map(path => h("button", { key: path, type: "button", className: `pathway-tab ${selectedPath === path ? "selected" : ""}`, onClick: () => { setSelectedPath(path); setLinkRequest(null); } }, path === "system" ? "⚙ System" : `${path === "solar" ? "☀" : path === "wind" ? "≋" : path === "hydro" ? "💧" : path === "biomass" ? "🌿" : "⛽"} ${pathwayLabels[path]}`))), linkRequest && linkOptions.length ? h("div", { className: "guided-subpanel trade-clean" }, h("strong", null, `Use a Knowledge Link for ${linkRequest.tech.name}`), h("p", null, `You use your Build action and pay the normal construction cost. The helping player receives one resource; neither player's Knowledge is spent.`), h("label", null, "Choose helper and payment", h("select", { value: selectedLink ? `${selectedLink.lenderId}|${selectedLink.paymentResource}` : "", onChange: event => setLinkRequest(current => ({ ...current, value: event.target.value })) }, ...linkOptions.map(option => h("option", { key: `${option.lenderId}-${option.paymentResource}`, value: `${option.lenderId}|${option.paymentResource}` }, `${option.lenderName} · pay 1 ${resourceLabels[option.paymentResource]}`)))), h("div", { className: "form-row compact" }, button("Go back", () => setLinkRequest(null), { kind: "ghost" }), button("Build with shared Knowledge", () => selectedLink && onKnowledgeLink(linkRequest.tech.id, selectedLink.lenderId, selectedLink.paymentResource), { kind: "primary", disabled: !selectedLink }))) : null, filtered.length === 0 ? h("p", { className: "empty-guidance" }, "No technology from this pathway is available in the market right now.") : null, h("div", { className: "clean-card-list" }, ...visible.map(tech => {
        ...
        return h("article", { key: tech.id, className: `clean-choice-card ${reachable ? "available" : "locked"}` }, h("div", { className: "clean-choice-main" }, h("div", { className: "choice-icon" }, tech.stage === "storage" ? "▣" : tech.stage === "transport" ? "⚡" : tech.stage === "research" ? "✦" : selectedPath === "solar" ? "☀" : selectedPath === "wind" ? "≋" : selectedPath === "hydro" ? "💧" : selectedPath === "biomass" ? "🌿" : selectedPath === "fossil" ? "⛽" : "⚙"), h("div", null, h("strong", null, tech.name), h("small", null, `${tierLabel} · Knowledge ${tech.knowledgeRequired} · Capacity ${tech.capacity}`))), h("div", { className: "cost-chips" }, badge(`${effectiveCost.constructionMaterials} General`, canAffordGeneral ? "" : "warning"), badge(`${effectiveCost.criticalMaterials} Critical`, canAffordCritical ? "" : "warning"), badge(`K${tech.knowledgeRequired}`)), legality.legal ? h("p", { className: "choice-benefit" }, tech.storage ? `Stores up to ${tech.storage.capacity} Energy.` : tech.fuel ? `Uses ${tech.fuel.units} ${resourceLabels[tech.fuel.resource]} to make up to ${tech.maximumOutput} Energy.` : `Can use up to ${tech.capacity} of this pathway's natural opportunity.`) : links.length ? h("p", { className: "choice-benefit" }, `${links.length} valid Knowledge Link option${links.length === 1 ? "" : "s"}. You still pay the normal build cost and one resource to the helper.`) : h(LockedReason, { reason: legality.reason }), h("div", { className: "choice-actions" }, infoButton(() => onInfo(technologyExplanation(tech)), `Explain ${tech.name}`), legality.legal ? button("Choose", () => onChoose({ kind: "build", technologyId: tech.id }), { kind: "primary compact" }) : links.length ? button("Use Knowledge Link", () => setLinkRequest({ tech, value: `${links[0].lenderId}|${links[0].paymentResource}` }), { kind: "secondary compact" }) : button("Locked", () => {}, { kind: "ghost compact", disabled: true, title: legality.reason })));
    })));
}
...
function ReviewScreen({ game, onContinue }) {
    return h("div", { className: "review" }, h("div", { className: "action-heading" }, h("div", null, h("p", { className: "eyebrow" }, "End-of-Generation review"), h("h2", null, `Generation ${game.generation}`)), button(game.generation === 8 ? "Calculate final results" : "Continue", onContinue, { kind: "primary" })), h("div", { className: "review-grid" }, ...currentOrder(game).map(id => {
        ...
        return h("article", { key: id, className: "review-card" }, h("h3", null, p.name), h("div", { className: "review-light" }, h("strong", null, m.deliveredLight), h("span", null, "Light")), h("div", { className: "mini-stats" }, stat("Need", m.reliabilityTarget), stat("Demand", m.reliabilityMet ? "Met" : "Missed"), stat("Point", m.reliabilityPointEarned ? "+1" : m.reliabilityPointCapped ? "Cap" : "—"), stat("Loss", sumLoss(m.systemLoss)), stat("Curtailment", m.curtailed), stat("Stored", m.storedEnd)), m.technologiesBuilt.length ? h("small", null, `Built: ${m.technologiesBuilt.map(t => getTechnology(game, t).name).join(", ")}`) : h("small", null, "No construction this Generation"));
    })));
}
...
function GameScreen({ game, setGame, onHome, onRules, onLoad }) {
    ...
    const knowledgeLink = (technologyId, lenderId, paymentResource) => { const borrower = currentPlayer(game); return command({ type: "knowledgeLinkBuild", borrowerId: borrower.id, lenderId, technologyId, paymentResource }); };
    ...
    const market = game.innovationMarket.visible.length
        ? panel("Advanced technology market", h("div", { className: "market-list" }, ...game.innovationMarket.visible.map(id => {
            ...
            return h("button", { type: "button", key: id, className: "market-mini-card", onClick: () => setInfo(technologyExplanation(tech)) }, h("strong", null, tech.name), h("small", null, `${tech.cost.constructionMaterials} General · ${tech.cost.criticalMaterials} Critical · Knowledge ${tech.knowledgeRequired}`));
        })), "nested")
