...
export function resolveFoundingProject(state, playerId, complete) {
    ...
    if (project.kind === "fuelNetwork") {
        ...
        player.resources.criticalMaterials.warehouse -= project.cost.criticalMaterials;
        if (player.resources.fossilFuel.currentContinent > 0 && warehouseTotal(player) < state.config.rules.warehouseMaximum) {
    ...
    switch (action.kind) {
        ...
        case "publicImport": { // World Market exchange: free action, fixed 2-for-1
            ...
            if (!Number.isInteger(required) || required !== 2)
                throw new Error("World Market exchange rate must be exactly 2-for-1.");
            ...
            log(state, "action.worldMarket", `${player.name} exchanged 2 Warehouse resources for 1 ${action.receive} without spending a Development action.`, player.id, { receive: action.receive, payment: structuredClone(payment), required });
            return false;
        }
        case "adapt": {
...
function createPlayer(config, setup) {
    ...
    const installed = startingIds.map((technologyId, index) => ({ instanceId: `${setup.id}-${technologyId}-${index + 1}`, technologyId, builtGeneration: 0, storageInput: emptyEnergy(), usedThisGeneration: false, firstOperationLossReduction: 0, temporaryCapacityBonus: 0 }));
    return {
        ...
        prepared: { pathwayId: null, capabilityId: null, pathwayUsed: false, capabilityUsed: false, foundingProjectResolved: false, foundingProjectCompleted: false, foundingProjectDeferred: false, fossilBlueprintAvailable: false },
...
export function createGame(config, players, seed, options = {}) {
    ...
    const state = {
        schemaVersion: "1.1.0", engineVersion: "0.16.0-regional-energy-chain",
...
function captureOutput(state, player, pathway) {
    ...
    const base = tableValue(table, Math.min(5, capacity));
    ...
    const withoutSignature = Math.max(0, Math.min(capacity, base + local));
    const output = Math.max(0, Math.min(capacity + signature, base + local + signature));
...
function addHydroInflow(state, player) {
    const reservoirs = player.installed.filter(instance => operational(state, instance) && getTechnology(state, instance.technologyId).storage?.type === "reservoir");
    if (reservoirs.length === 0)
        return;
    const turbineCapacity = player.installed
        .filter(instance => operational(state, instance))
        .map(instance => getTechnology(state, instance.technologyId))
        .filter(technology => technology.pathway === "hydro" && technology.stage === "transformation")
        .reduce((sum, technology) => sum + technology.capacity, 0);
    let inflow = tableValue(state.config.weather.hydro[state.weather.current], Math.min(5, turbineCapacity));
    ...
    if (signature > 0) {
        ...
        player.currentMetrics.continentAbilityActivations++;
        log(state, "continent.signature", `${player.name}'s Hydro signature added ${signature} inflow.`, player.id, { pathway: "hydro", value: signature });
    }
    for (const reservoir of reservoirs) {
        if (inflow <= 0) break;
        const technology = getTechnology(state, reservoir.technologyId);
        const room = storageCapacity(state, player, reservoir, technology) - totalEnergy(reservoir.storageInput);
        const amount = Math.max(0, Math.min(room, inflow));
        reservoir.storageInput.hydro += amount;
        inflow -= amount;
    }
}
...
function biomassRegrowth(state, player) {
    ...
    if (condition?.effect.kind === "hydroDelta" && condition.effect.fallbackBiomassRegrowthDelta !== undefined)
...
function migratePhase3State(input) {
    ...
    // Canonical A.5.8 configuration replaces conflicting historic runtime values.
    state.config = structuredClone(defaultConfig);
    state.schemaVersion = "1.1.0";
    state.engineVersion = "0.16.0-regional-energy-chain";
    ...
    for (const player of Object.values(state.players)) {
        ...
        player.cumulative.demandMetGenerations ??= Object.entries(player.lightByGeneration ?? {}).filter(([generation, light]) => light >= (state.config.demand.reliabilityTargets[Number(generation)] ?? 0)).length;
...
export function deserializeGame(json) {
    ...
    if (!["1.0.0", "1.1.0"].includes(envelope.saveSchemaVersion))
