...
import { createModeConfig, featureEnabled, modeAllowsTechnology, modeOpeningMode, normalizeGameMode } from "./modes.js";
...
function availableForBuild(state, technology) { return modeAllowsTechnology(state, technology) && (technology.alwaysAvailable || state.innovationMarket.visible.includes(technology.id)); }
...
function build(state, player, technologyId, options = {}) {
    ...
    if (featureEnabled(state, "fullRegionalRules") && profile.resourceRule?.kind === "circularRecovery" && !player.circularRecovery.usedThisGeneration && !player.circularRecovery.pendingResource) {
        const spent = [];
...
export function performDevelopmentAction(state, playerId, action) {
    ...
    switch (action.kind) {
        ...
        case "harvestBiomass": { // retained command alias for older saves/UI; canonical rule is extraction
            ...
            const specialtyAvailable = featureEnabled(state, "fullRegionalRules")
                && profile.resourceRule?.kind === "extraction"
        ...
        case "research": {
            if (!featureEnabled(state, "knowledgeRequirements"))
                throw new Error("Learning is introduced in Intermediate mode.");
...
function createMarket(config, state) {
    const advanced = shuffle(config.technologies.filter(t => !t.starter && !t.alwaysAvailable && modeAllowsTechnology(config, t)).map(t => t.id), state.streams.market);
...
export function createGame(config, players, seed, options = {}) {
    ...
    const gameMode = normalizeGameMode(options.gameMode ?? config.gameMode?.id ?? "master");
    const openingMode = modeOpeningMode(gameMode, options.openingMode ?? config.opening?.defaultMode ?? "startingPlan");
    const state = {
        schemaVersion: "1.7.0", engineVersion: "0.24.0-layered-game-modes",
        gameMode,
    ...
    log(state, "game.created", `Created ${config.gameMode?.label ?? "SUNPATHS"} game with ${players.length} players and seed ${seed}.`, null, { openingMode, gameMode });
    return state;
}
// -----------------------------------------------------------------------------
...
function resolveCircularRecovery(state, player) {
    ...
    if (!featureEnabled(state, "fullRegionalRules") || profile.resourceRule?.kind !== "circularRecovery" || !resource || player.circularRecovery.usedThisGeneration)
        return null;
    const account = player.resources[resource];
...
function beginGeneration(state) {
    ...
    if (featureEnabled(state, "globalEvents"))
    ...
    state.phase = featureEnabled(state, "localConditions") ? "generation.localConditions" : "generation.development";
...
function applyCommandMutable(state, command) {
    switch (command.type) {
        ...
        case "drawLocalConditions":
            if (!featureEnabled(state, "localConditions"))
                throw new Error("Local Conditions are not used in this game mode.");
...
export function applyCommand(state, command) {
    ...
    if ((command.type === "drawLocalConditions" || command.type === "beginGeneration") && draft.phase === "generation.development")
...
function migratePhase3State(input) {
    ...
    const sourceMode = normalizeGameMode(state.gameMode ?? state.config?.gameMode?.id ?? "master");
    // Older saves migrate to the complete Master rules. Layered-mode saves retain their configuration snapshot.
    const sourceConfig = sourceSchemaVersion === "1.7.0" && state.config ? state.config : defaultConfig;
    state.config = createModeConfig(sourceConfig, sourceMode);
    state.gameMode = sourceMode;
    state.schemaVersion = "1.7.0";
    state.engineVersion = "0.24.0-layered-game-modes";
    ...
    for (const player of Object.values(state.players)) {
        ...
        player.globalEventUsage ??= { buildUsed: false, worldMarketUsed: false, biomassPenaltyUsed: false };
        if (!["1.5.0", "1.6.0", "1.7.0"].includes(sourceSchemaVersion)) {
    ...
    if (!["1.6.0", "1.7.0"].includes(sourceSchemaVersion) && ["setup.summit", "setup.foundingProjects", "setup.revealPrepared"].includes(state.phase)) {
...
export function deserializeGame(json) {
    ...
    if (!["1.0.0", "1.1.0", "1.2.0", "1.3.0", "1.4.0", "1.5.0", "1.6.0", "1.7.0"].includes(envelope.saveSchemaVersion))
