...
import { activeGlobalEvent, conditionApplies, directTradeBlocked, getCondition, getGlobalEvent, globalEventDrawsThisGeneration, countInstalled, emptyEnergy, getPlayer, getTechnology, log, resourceTypes, applyCompletedUpgradeConsequences, getEffectiveUpgradeCost, getKnowledgeRequirement, emptyMetrics, getStartingTechnologyIds, setOpeningWeather, revealPreparedAndBeginFounding, addEnergy, fuelPlantMaximumOutput, fossilChainSnapshot, hasRelevantSystem, hasTechnology, pathways, totalEnergy, getContinentGenerationModifiers, gatherAmount, discardCurrentConditions, drawLocalConditions, finalRanking, advanceWeather, setInitialCurrent, setInitialForecast, assertInvariants, getContinentProfile, warehouseTotal, worldMarketBlocked, worldMarketRate } from "./rules.js";
...
export function createGame(config, players, seed, options = {}) {
    ...
    const state = {
        schemaVersion: "1.4.0", engineVersion: "0.21.0-streamlined-opening",
...
function migratePhase3State(input) {
    ...
    state.engineVersion = "0.21.0-streamlined-opening";
    ...
    // A.5.14 removes the reveal and opening-die interstitials. Preserve completed Founding Projects,
    // but make any unfinished opening state use one Generation-1 Current / Generation-2 Forecast pair.
    if (state.phase === "setup.revealPrepared") {
        setOpeningWeather(state);
        revealPreparedAndBeginFounding(state);
    }
...
export function deserializeGame(json) {
    ...
    if (!["1.0.0", "1.1.0", "1.2.0", "1.3.0", "1.4.0"].includes(envelope.saveSchemaVersion))
