...
import { conditionApplies, countInstalled, fuelPlantMaximumOutput, fossilChainSnapshot, getTechnology, hasTechnology, pathways, totalEnergy, getContinentGenerationModifiers, getEffectiveUpgradeCost, getKnowledgeRequirement } from "./rules.js";
// -----------------------------------------------------------------------------
...
export function getReliabilityTarget(state, playerId, generation = state.generation) {
    const required = getRequiredLight(state, playerId, generation);
    return Math.min(state.config.demand.maximumLight, required + 1);
}
...
export function getEnergyChainBreakdown(state, playerId) {
    ...
    const generated = preview?.grossEnergy ?? Math.max(0, snapshot.availableNow + storageLoss + transformationLoss);
    const beforeGrid = Math.max(0, generated - storageLoss - transformationLoss);
    const deliveredEnergy = preview?.transported ?? Math.min(beforeGrid, snapshot.gridCapacity);
    ...
    const unusedEnergy = preview?.curtailed ?? Math.max(0, beforeGrid - deliveredEnergy);
    return {
        generated,
        weatherBonus: getContinentGenerationModifiers(state, playerId, "solar").generationBonus + getContinentGenerationModifiers(state, playerId, "wind").generationBonus + getContinentGenerationModifiers(state, playerId, "hydro").generationBonus,
        storageLoss,
        transformationLoss,
        beforeGrid,
        ...
        sourceByPathway: {
            ...
            hydro: snapshot.hydro.available,
