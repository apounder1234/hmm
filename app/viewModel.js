...
import { featureEnabled, modeAllowsTechnology } from "./modes.js";
// -----------------------------------------------------------------------------
...
export function technologyImpactPreview(state, playerId, technologyOrId) {
    ...
    const forecastFace = featureEnabled(state, "forecastVisible") ? state.weather.forecast : null;
    ...
    const desired = playerId
        && after.pointTarget != null
        && state.players[playerId].cumulative.reliableGenerations < (state.config.rules.reliabilityPointMaximum ?? 4)
        ? after.pointTarget
        : after.target;
...
export function buildLegality(state, playerId, technology, options = {}) {
    if (!modeAllowsTechnology(state, technology))
        return { legal: false, reason: `${technology.name} is introduced in a later game mode.` };
...
export function technologyReadiness(state, playerId, technologyOrId, options = {}) {
    ...
    const blockers = [];
    if (!modeAllowsTechnology(state, technology))
        blockers.push({ kind: "mode", label: `${technology.name} is introduced in a later game mode.` });
