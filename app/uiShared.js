...
import { gameModes, getGameMode, createModeConfig, featureEnabled, modeAllowsTechnology, modeOpeningMode, defaultCapabilityForPathway } from "./modes.js";
...
const abilityDescriptions = {
    criticalMaterialsExtraction: { name: "Critical Materials Specialty", text: "Once per Generation, extracting Critical Materials yields 2 instead of the normal 1." },
    ...
    otherMaterialsExtraction: { name: "Other Materials Specialty", text: "Once per Generation, extracting Other Materials yields 2 instead of the normal 1." },
    fuelExtraction: { name: "Fuel Extraction Specialty", text: "Once per Generation, extracting Fossil Fuel yields 2 instead of the normal 1." },
    biomassExtraction: { name: "Biomass Extraction Specialty", text: "Once per Generation, extracting Biomass yields 2 instead of the normal 1." }
};
...
const stageDescriptions = {
    ...
    Store: "Keeps Energy for a later Generation. Energy charged now cannot be released until the next Generation.",
    ...
    Light: "Meets the Generation need. Reliability Points are earned by meeting demand in Generations 5–8."
};
...
function humanizeWeatherIds(text) {
    let result = String(text ?? "");
    for (const [id, label] of Object.entries(weatherLabels))
        result = result.replace(new RegExp(`\\b${id}\\b`, "g"), label);
    return result;
}
function technologyBenefitExplanation(game, player, tech) {
    const impact = technologyImpactPreview(game, player.id, tech);
    const timingLabel = impact.timing === "Helps now"
        ? "Helps your system now"
        : impact.timing === "Helps next forecast"
            ? "Helps in the next forecast"
            : impact.timing === "Future upgrade"
                ? "Shows the value of this later tier"
                : "Prepares a later part of your system";
    const details = [];
    if (impact.comparisonMode === "tier")
        details.push("Because the required earlier tier is not installed yet, this preview compares that earlier tier with this technology.");
    details.push(`Now: ${humanizeWeatherIds(impact.now)}`);
    details.push(`Later: ${humanizeWeatherIds(impact.future)}`);
    for (const metric of impact.metrics)
        details.push(`${metric.label}: ${metric.before}${metric.unit} → ${metric.after}${metric.unit}.`);
    details.push(`What to improve next: ${humanizeWeatherIds(impact.prompt)}`);
    return {
        eyebrow: timingLabel,
        title: tech.name,
        summary: impact.headline === "Future flexibility improves" && tech.gameBenefit ? tech.gameBenefit : impact.headline,
        status: impact.timing,
        details
    };
}
...
function scoringExplanation(game, generation, value, target) {
    const eligible = generation >= (game.config.rules.reliabilityStartsGeneration ?? 5);
    return {
        eyebrow: `Generation ${generation}`,
        title: eligible ? "Light and Reliability" : "Light",
        summary: eligible ? "Meeting demand in this Generation earns one Reliability Point." : "Reliability Points begin in Generation 5.",
        details: [
            `Recorded Light: ${value ?? "not resolved yet"}.`,
            `Light needed: ${target}.`,
            eligible ? "Meet or exceed the requirement to earn one Reliability Point." : "This Generation still counts toward total Light and demand history, but not Reliability Points.",
            `Each player can earn at most ${game.config.rules.reliabilityPointMaximum ?? 4} Reliability Points.`
        ]
    };
}
function globalEventExplanation(game) {
    const event = activeGlobalEvent(game);
    if (!event)
        return null;
    const effect = event.effect;
    const summaries = {
        tradeShutdown: "Direct player trade and the World Market are unavailable this Generation.",
        worldMarketResourceBlocked: `${resourceLabels[effect.resource]} cannot be obtained from the World Market this Generation.`,
        extractionYieldCap: `${resourceLabels[effect.resource]} extraction is capped at ${effect.maximumYield} this Generation.`,
        firstBuildResourceDelta: `Each player's first Build costs ${effect.amount} additional Other Material.`,
        worldMarketRateOverride: `World Market ${resourceLabels[effect.resource]} costs ${effect.rate} resources for one this Generation.`,
        harvestFailure: "Biomass cannot be obtained from the World Market and Biomass regrowth is zero this Generation.",
        firstBiomassOutputDelta: `Each player's first Biomass operation produces ${Math.abs(effect.amount)} less Energy.`,
        firstWorldMarketRate: `Each player's first World Market exchange costs ${effect.rate} resource for one this Generation.`
    };
    return {
        eyebrow: "Global Event",
        title: event.name,
        summary: summaries[effect.kind] || "A temporary worldwide effect is active.",
        details: ["This affects all players for the current Generation only."]
    };
}
...
function button(label, onClick, options = {}) {
    return h("button", { type: "button", className: `button ${options.kind || ""}`, onClick, disabled: Boolean(options.disabled), title: options.title, "aria-label": options.ariaLabel }, label);
}
...
function friendlyActionName(action, game) {
    ...
    if (action.kind === "extract")
        return `Extract ${resourceLabels[action.resource]}`;
...
function actionConfirmation(game, player, action) {
    ...
    if (action.kind === "extract" || action.kind === "harvestBiomass") {
        ...
        return {
            ...
            whyItMatters: amount > (game.config.resources?.normalExtractionYield ?? 1) ? `${game.config.continents.find(item => item.id === player.continentId).name}'s once-per-Generation specialty makes this extraction more efficient. Materials enable construction but do not produce Light by themselves.` : "This resource can support future construction or operation, but it does not produce Light by itself.",
...
export const uiShared = {
    ...
    technologyBenefitExplanation,
    resourceExplanation,
    scoringExplanation,
    globalEventExplanation,
    clone,
    number,
    titleCase,
    sumLoss,
