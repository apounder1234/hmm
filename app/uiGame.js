...
function EnergyChain({ game, player, onInfo, planning = true }) {
    const chain = getEnergyChainBreakdown(game, player.id);
    const stages = [
        { label: "Source", value: chain.generated, detail: `${Object.entries(chain.sourceByPathway).filter(([, value]) => value > 0).map(([path, value]) => `${pathwayLabels[path]} ${value}`).join(" · ") || "No source can operate"}` },
        { label: "Storage / transformation", value: chain.beforeGrid, detail: `${chain.storageLoss + chain.transformationLoss} Energy lost` },
        { label: "Grid", value: chain.deliveredEnergy, detail: `Capacity ${chain.gridCapacity} · ${chain.unusedEnergy} unused` },
        { label: "Lighting", value: chain.lightProduced, detail: `${chain.lightingLoss} Energy not converted to Light` }
    ];
    const summary = `${chain.generated} Energy generated. ${chain.storageLoss + chain.transformationLoss} Energy lost before the Grid. ${chain.deliveredEnergy} Energy transported. ${chain.lightProduced} Light possible.`;
    return h("div", { className: "energy-chain educational-chain", role: "group", "aria-label": summary },
        ...stages.flatMap((stage, index) => [
            h("button", { type: "button", key: stage.label, className: "energy-stage explainable", onClick: () => onInfo({ eyebrow: "Energy chain", title: stage.label, summary: stage.detail, details: [summary, planning ? "These are current planning values using the same rules as Dispatch." : "These values describe the resolved Generation."] }) }, h("strong", null, stage.label), h("b", null, stage.value), h("small", null, stage.detail), h("i", { className: "micro-help" }, "?")),
            index < stages.length - 1 ? h("span", { key: `${stage.label}-arrow`, className: "chain-arrow" }, "→") : null
        ].filter(Boolean)),
...
function PlayerBoard({ game, player, onInfo }) {
    ...
    const knowledgeButton = h("button", {
        ...
        onClick: () => onInfo({
            ...
            title: player.temporaryKnowledge ? `Knowledge ${player.knowledge} + ${player.temporaryKnowledge} temporary` : `Knowledge ${player.knowledge}`,
            summary: player.temporaryKnowledge ? `Effective construction Knowledge is ${effectiveKnowledge} this Generation.` : "Knowledge unlocks the technology ladder for this player.",
            details: [
                ...
                player.temporaryKnowledge ? `Engineering Exchange or another temporary effect adds ${player.temporaryKnowledge} only for Build legality this Generation.` : "Knowledge is never spent. Increase it through Learn actions to unlock your own advanced technologies."
            ]
        })
    }, player.temporaryKnowledge ? `Knowledge ${player.knowledge} +${player.temporaryKnowledge}` : `Knowledge ${player.knowledge}`, h("i", { className: "micro-help" }, "?"));
    const statusBadges = [
        knowledgeButton,
...
function GatherPanel({ game, player, onChoose }) {
    const choices = [
        { action: { kind: "extract", resource: "constructionMaterials" }, icon: "▦", title: "Other Materials", note: "Construction, machinery and general industrial inputs." },
        { action: { kind: "extract", resource: "criticalMaterials" }, icon: "◆", title: "Critical Minerals", note: "Specialised mineral inputs for advanced systems." },
        { action: { kind: "extract", resource: "fossilFuel" }, icon: "●", title: "Fossil Fuel", note: "Dispatchable input for the fossil Energy chain." },
        { action: { kind: "extract", resource: "biomass" }, icon: "♣", title: "Biomass", note: "Recurring feedstock for managed Biomass plants." }
    ];
    return h("div", { className: "resource-site-grid" }, ...choices.map(choice => {
        const legality = developmentActionLegality(game, player.id, choice.action);
        const key = choice.action.resource;
        const account = player.resources[key];
        const amount = gatherAmount(game, player, key);
        const specialty = amount > (game.config.resources?.normalExtractionYield ?? 1);
        return h("article", { key, className: `resource-site ${legality.legal ? "available" : "locked"} ${specialty ? "specialty" : ""}` },
            h("div", { className: "resource-site-heading" }, h("span", null, choice.icon), h("div", null, h("h3", null, choice.title), h("small", null, choice.note))),
            specialty ? h("strong", { className: "regional-yield" }, `Regional specialty · extract ${amount}`) : h("strong", { className: "normal-yield" }, `Normal extraction · gain ${amount}`),
            h("div", { className: "resource-site-stats" },
                h("span", null, h("small", null, "Warehouse"), h("b", null, account.warehouse)),
                h("span", null, h("small", null, "Regional stock"), h("b", null, account.currentContinent)),
                h("span", null, h("small", null, "Total space"), h("b", null, `${warehouseTotal(player)}/${game.config.rules.warehouseMaximum}`))
            ),
            legality.legal ? h("p", { className: "choice-benefit" }, `Spend 1 action: ${account.warehouse} → ${account.warehouse + amount} ${choice.title}.`) : h(LockedReason, { reason: legality.reason }),
            button(legality.legal ? `Place action token · gain ${amount}` : "Unavailable", () => onChoose(choice.action), { kind: legality.legal ? "primary full" : "ghost full", disabled: !legality.legal, title: legality.reason })
        );
    }));
}
function LearnPanel({ game, player, onChoose }) {
    ...
    return h("div", { className: "knowledge-board" },
        ...
        legality.legal ? button(`Place action token · reach Knowledge ${nextPermanent}`, () => onChoose({ kind: "research" }), { kind: "primary full" }) : h(LockedReason, { reason: legality.reason })
    );
}
...
function TradePanel({ game, player, onTrade, message }) {
    ...
    const canPropose = Boolean(recipient) && player.actionsRemaining > 0 && !sameDirectionResource && enough;
    const reason = player.actionsRemaining <= 0 ? "No Development action remains." : sameDirectionResource ? "The same resource cannot move in both directions." : !enough ? "One side lacks an offered resource." : "Available";
...
function DevelopmentControls({ game, player, onAction, onUndo, onReset, onTrade, tradeMessage, onInfo }) {
    ...
    const [showIdea, setShowIdea] = useState(game.generation <= 2);
    const [bottleneckGuess, setBottleneckGuess] = useState(null);
    useEffect(() => { setSection(null); setShowIdea(game.generation <= 2); setBottleneckGuess(null); }, [player.id, game.actionRound, game.generation]);
    const condition = conditionDefinition(game, player);
    ...
    const goalStatus = maximum >= resilience ? "Resilient" : maximum >= required ? "Secure" : "At Risk";
    ...
    const recommendation = useMemo(() => {
        if (!guidedMode)
            return null;
        try { return chooseDevelopmentDecision(game, player); }
        catch { return null; }
    ...
    const choose = action => {
        ...
        if (!legality.legal) {
            onInfo({ eyebrow: "Action unavailable", title: friendlyActionName(action, game), summary: legality.reason, details: ["No resources or Development actions were consumed."] });
            return;
        }
    ...
    const actionCards = [
        { id: "gather", icon: "▦", title: "Extract", text: "Move a physical resource from regional stock into your Warehouse.", count: usage.gather, available: player.actionsRemaining > 0, reason: player.actionsRemaining > 0 ? null : "No Development action remains." },
        { id: "learn", icon: "✦", title: "Learn", text: "Advance permanent Knowledge and unlock designs.", count: usage.learn, available: researchStatus.legal, reason: researchStatus.legal ? null : researchStatus.reason },
        { id: "build", icon: "⚙", title: "Build", text: "Change generation, storage, Grid, Lighting or transformation.", count: usage.build, available: player.actionsRemaining > 0, reason: player.actionsRemaining > 0 ? null : "No Development action remains." },
        { id: "trade", icon: "⇄", title: "Trade / Market", text: "Negotiate value-preserving trade or use the fast 2-for-1 World Market.", count: usage.trade, freeCount: usage.market, available: true, reason: null }
    ];
    const sectionTitles = { gather: "Extract from your region", learn: "Advance Knowledge", build: "Develop your Energy system", trade: "Trade and World Market" };
    return h("div", { className: "action-panel clean-action-panel player-focus-panel development-board" },
        h("div", { className: "turn-banner simplified-turn-banner" },
            h("div", null, h("p", { className: "eyebrow" }, `Generation ${game.generation} · action round ${game.actionRound}`), h("h2", null, `${player.name}'s Development Board`)),
            h("div", { className: "development-token-rack", "aria-label": `${player.actionsRemaining} actions remaining` },
                ...Array.from({ length: game.config.rules.actionsPerGeneration }, (_, index) => h("span", { key: index, className: `development-token ${index >= player.actionsRemaining ? "spent" : "ready"}` }, index >= player.actionsRemaining ? "✓" : "●")),
                h("strong", null, `${player.actionsRemaining} action${player.actionsRemaining === 1 ? "" : "s"} left`)
            )
        ),
        h("section", { className: `generation-goal-card status-${goalStatus.toLowerCase().replace(" ", "-")}` },
            h("div", { className: "goal-status-heading" }, h("div", null, h("small", null, "KEEP THE LIGHTS ON"), h("strong", null, `Required this Generation: ${required} Light`)), h("b", null, goalStatus)),
            h("div", { className: "goal-three-way" },
                h("div", null, h("small", null, "REQUIRED LIGHT"), h("strong", null, required), h("span", null, "Meet this to keep demand supplied.")),
                h("div", null, h("small", null, "RESILIENCE TARGET"), h("strong", null, resilience), h("span", null, "Reach this to earn a Reliability Point.")),
                h("div", null, h("small", null, "MAXIMUM LIGHT POSSIBLE RIGHT NOW"), h("strong", null, maximum), h("span", null, maximum >= required ? "Demand is reachable." : "The current system cannot yet meet demand."))
            )
        ),
        h("section", { className: "development-energy-focus" },
            h("div", { className: "education-section-heading" }, h("div", null, h("small", null, "YOUR ENERGY SYSTEM NOW"), h("strong", null, "Follow the chain from source to Light")), h("span", null, weatherLabels[game.weather.current] ?? "No weather")),
            h(EnergyChain, { game, player, onInfo }),
            h("div", { className: "bottleneck-constraint-grid" },
                h("article", { className: `bottleneck-card type-${bottleneck.type}` }, h("small", null, bottleneck.type === "none" ? "SYSTEM READY" : "CURRENT BOTTLENECK"), h("strong", null, bottleneck.label), h("p", null, bottleneck.explanation)),
                h("article", { className: "constraint-card" }, h("small", null, "DEVELOPMENT CONSTRAINT"), h("strong", null, constraint.label), h("p", null, constraint.explanation))
            ),
            guidedMode && game.generation < 6 && recommendation ? h("div", { className: "possible-response" }, button(showIdea ? "Hide idea" : "Show an idea", () => setShowIdea(value => !value), { kind: "ghost compact" }), showIdea ? h("p", null, h("strong", null, "One possible response: "), friendlyActionName(recommendation.action, game)) : null) : null,
            guidedMode && game.generation >= 6 ? h("details", { className: "late-bottleneck-prompt" },
                h("summary", null, "What do you think is limiting the system?"),
                h("p", { className: "muted" }, "Optional reflection — it does not affect score or available actions."),
                h("div", { className: "bottleneck-guess-buttons" }, ...["Generation", "Storage", "Grid", "Lighting", "Resources"].map(label => button(label, () => setBottleneckGuess(label), { kind: bottleneckGuess === label ? "secondary compact" : "ghost compact" }))),
                bottleneckGuess ? h("p", { className: "guess-reveal", "aria-live": "polite" }, h("strong", null, `You selected ${bottleneckGuess}. `), `The current calculation identifies ${bottleneck.label.toLowerCase()}: ${bottleneck.explanation}`) : null
            ) : null
        ),
        h(ActiveConditionStrip, { game, player, condition, onInfo }),
        adaptable && !section ? h("button", { type: "button", className: "adapt-alert", onClick: () => choose({ kind: "adapt" }), disabled: !adaptStatus?.legal }, h("strong", null, `Respond to ${condition.name}`), h("span", null, "Place one action token to cancel its penalty.")) : null,
        !section ? h("div", { className: "development-action-spaces" }, ...actionCards.map(card => h("button", { key: card.id, type: "button", className: `development-action-space ${card.available ? "" : "needs-work"}`, title: card.reason ?? "Open this action space.", onClick: () => setSection(card.id) },
            h("span", { className: "action-space-icon", "aria-hidden": "true" }, card.icon), h("div", null, h("strong", null, card.title), h("small", null, card.text)),
            h("div", { className: "action-space-usage" }, card.count ? h("b", null, `${card.count} token${card.count === 1 ? "" : "s"} placed`) : h("b", null, card.available ? "Available now" : "Open to see requirements"), card.freeCount ? h("em", null, `${card.freeCount} action-free market exchange${card.freeCount === 1 ? "" : "s"}`) : null),
            !card.available && card.reason ? h("em", { className: "action-space-reason" }, card.reason) : null
        ))) : null,
        section ? h("div", { className: "section-step development-section" },
            h("div", { className: "section-step-heading" }, button("← Development Board", () => setSection(null), { kind: "ghost" }), h("h3", null, sectionTitles[section])),
            ...
            section === "trade" ? h("div", { className: "trade-sections" },
                h("section", { className: "trade-mode-card" }, h("h4", null, "World Market"), h("p", null, "Action cost: 0. Pay exactly 2 Warehouse resources for 1 market resource."), h(ImportChoices, { game, player, onChoose: choose })),
                h("section", { className: "trade-mode-card" }, h("h4", null, "Player Trade"), h("p", null, "The initiating player uses one Development action; a negotiated bundle can preserve more resource value."), h(TradePanel, { game, player, onTrade, message: tradeMessage }))
            ) : null
        ) : null,
        h("details", { className: "turn-utility-controls" }, h("summary", null, "Turn controls"), h("div", { className: "safe-controls" }, button("Pass", () => choose({ kind: "pass" }), { kind: "ghost" }), button("Undo last confirmed action", onUndo, { kind: "ghost", disabled: game.undo.stack.length === 0 }), button("Reset Generation", onReset, { kind: "ghost", disabled: !game.undo.generationStart }))),
        confirmation ? h(ConfirmationModal, { confirmation, onConfirm: confirm, onCancel: () => setConfirmation(null) }) : null
    );
}

function DispatchPanel({ game, player, onDispatch, onInfo, prediction, onPrediction }) {
    ...
    const storageLoss = preview.fossilChain?.storageLoss ?? 0;
    const transformationLoss = Math.max(0, (preview.lossBreakdown?.thermal ?? 0) - storageLoss);
    const lightingLoss = preview.lossBreakdown?.lighting ?? 0;
    const deliveredEnergy = preview.transported;
    const significantChange = Boolean(player.currentMetrics.technologiesBuilt?.length || condition || game.generation <= 5);
    const [optionalPredictionOpen, setOptionalPredictionOpen] = useState(false);
    const predictionRequested = prediction === undefined && (significantChange || optionalPredictionOpen);
    const fuelText = Object.entries(preview.fuelConsumed ?? {}).filter(([, value]) => value > 0).map(([resource, value]) => `${value} ${resourceLabels[resource]}`).join(" · ");
    const bottleneck = getPrimaryBottleneck(game, player.id, { target: preview.target });
    const predictionPanel = h("section", { className: "prediction-card", "aria-label": "Optional Light prediction" },
        h("p", { className: "eyebrow" }, game.generation <= 2 ? "Pause and predict" : "Quick prediction"),
        h("h3", null, "How much Light do you think will reach the community?"),
        h("p", null, "This costs nothing, has no effect on score and is reviewed only after the Energy phase."),
        h("div", { className: "prediction-chain-clues" },
            h("span", null, h("small", null, "Sources ready"), h("strong", null, sourceRows.map(([path]) => pathwayLabels[path]).join(" + ") || "None")),
            h("span", null, h("small", null, "Grid capacity"), h("strong", null, preview.gridCapacity)),
            h("span", null, h("small", null, "Lighting maximum"), h("strong", null, preview.lightingMaximum)),
            h("span", null, h("small", null, "Required Light"), h("strong", null, preview.target))
        ),
        h("div", { className: "prediction-buttons", role: "radiogroup", "aria-label": "Predicted Light" }, ...Array.from({ length: game.config.demand.maximumLight + 1 }, (_, value) => h("button", { key: value, type: "button", role: "radio", "aria-checked": false, onClick: () => onPrediction(value) }, value))),
        button("Skip prediction", () => onPrediction(null), { kind: "ghost" })
    );
    if (predictionRequested)
        return h("div", { className: "action-panel dispatch-clean" }, h("div", { className: "turn-banner" }, h("div", null, h("p", { className: "eyebrow" }, "Before Energy resolution"), h("h2", null, `${player.name}'s Energy prediction`)), badge(weatherLabels[game.weather.current], "weather")), predictionPanel);
    const predictionLabel = prediction === null ? "Prediction skipped" : prediction === undefined ? "No prediction made" : `Predicted ${prediction} Light`;
    return h("div", { className: "action-panel dispatch-clean" },
        ...
        prediction === undefined && !significantChange ? h("div", { className: "optional-prediction" }, h("span", null, "Your system has not changed significantly."), button("Make an optional prediction", () => setOptionalPredictionOpen(true), { kind: "ghost compact" })) : h("p", { className: "prediction-recorded" }, predictionLabel),
        ...
        h("div", { className: "source-breakdown" }, h("strong", null, "Generated Energy by source"), sourceRows.length ? h("div", { className: "source-chips" }, ...sourceRows.map(([pathway, value]) => badge(`${pathwayLabels[pathway]} ${value}`, "energy"))) : h("small", null, "No Energy source can operate in this plan."), fuelText ? h("small", null, `Recurring resource consumed: ${fuelText}.`) : null),
        h("div", { className: "visual-flow educational-resolution", "aria-live": "polite" },
            h("div", { className: "flow-node" }, h("small", null, "GENERATED"), h("strong", null, `${preview.grossEnergy} Energy`), h("span", null, sourceRows.map(([path, value]) => `${pathwayLabels[path]} ${value}`).join(" · ") || "No generation")),
            h("div", { className: "flow-arrow" }, "→"),
            h("div", { className: "flow-node" }, h("small", null, "STORAGE / TRANSFORMATION"), h("strong", null, `${Math.max(0, preview.grossEnergy - storageLoss - transformationLoss)} remain`), h("span", null, `Storage loss −${storageLoss} · transformation loss −${transformationLoss}`)),
            h("div", { className: "flow-arrow" }, "→"),
            h("div", { className: "flow-node" }, h("small", null, "GRID"), h("strong", null, `${deliveredEnergy} Energy delivered`), h("span", null, `Capacity ${preview.gridCapacity} · ${preview.curtailed} unused or stored`)),
            h("div", { className: "flow-arrow" }, "→"),
            h("div", { className: `flow-node light-result ${preview.pointEarned ? "reliable" : preview.demandMet ? "demand-met" : ""}` }, h("small", null, "LIGHTING"), h("strong", null, `${preview.light} Light`), h("span", null, lightingLoss ? `${lightingLoss} delivered Energy did not become Light` : "No Lighting conversion loss"))
        ),
        h("div", { className: "dispatch-explanation" }, h("strong", null, bottleneck.type === "none" ? "System ready" : "Current bottleneck"), h("p", null, bottleneck.explanation)),
        h("div", { className: "dispatch-metrics" }, stat("Required Light", preview.target), stat("Resilience target", Math.min(game.config.demand.maximumLight, preview.target + 1)), stat("Stored after", preview.stored), stat("Unused Energy", preview.curtailed)),
...
function ReviewScreen({ game, onContinue, predictions }) {
    ...
    const renderReviewCard = (id, compact = false) => {
        ...
        const condition = conditionDefinition(game, player);
        const prediction = predictions?.[`${game.generation}:${id}`];
        ...
        const content = h("article", { className: `review-story-card ${story.demandMet ? "goal-met" : "goal-missed"}` },
            h("div", { className: "review-story-heading" }, h("div", null, h("small", null, player.name), h("h3", null, game.config.continents.find(continent => continent.id === player.continentId).name)), h("span", { className: story.demandMet ? "review-status success" : "review-status warning" }, story.demandMet ? story.pointEarned ? "Resilient" : "Demand met" : "Demand not fully met")),
            h("div", { className: "review-goal-summary" }, stat("Required", `${story.requiredLight} Light`), stat("Predicted", prediction === undefined || prediction === null ? "—" : `${prediction} Light`), stat("Delivered", `${story.lightProduced} Light`)),
...
function FocusStrip({ game, player, onInfo }) {
    ...
    const condition = conditionDefinition(game, player);
    const status = maximum >= resilience ? "Resilient" : maximum >= required ? "Secure" : "At Risk";
    return h("section", { className: `focus-strip status-${status.toLowerCase().replace(" ", "-")}`, "aria-label": "Current game focus" },
        ...
        h("div", { className: "focus-goal" }, h("small", null, "Required Light"), h("strong", null, `💡 ${required}`), h("span", null, `Resilience target ${resilience}`)),
...
function GameScreen({ game, setGame, onHome, onCards, onRules, onLoad }) {
    ...
    const actionFeedback = (player, action) => {
        ...
        if (action.kind === "publicImport")
            return { title: `${resourceLabels[action.receive]} received from the World Market`, summary: "You paid two Warehouse resources for one World Market resource without spending a Development action.", metrics: [], prompt: "Check Build again to see which complete costs are now affordable." };
    ...
    if (game.phase.startsWith("setup."))
        ...
        decision = panel("A new Generation begins", h("div", { className: "generation-start-card" }, h("p", null, `Today: ${weatherLabels[game.weather.current]}. Next forecast: ${game.weather.forecast ? weatherLabels[game.weather.forecast] : "none"}.`), h("div", { className: "generation-start-goals" }, h("strong", null, `Required Light: ${game.config.demand.reliabilityTargets[game.generation || 1]}.`), h("span", null, `Optional resilience target: ${Math.min(game.config.demand.maximumLight, game.config.demand.reliabilityTargets[game.generation || 1] + 1)} Light for a Reliability Point.`)), button("Start Generation", () => command({ type: "beginGeneration" }), { kind: "primary large" })));
        ...
        const active = currentPlayer(game);
        decision = active?.controller.kind === "human" ? h(DispatchPanel, { game, player: active, onDispatch: dispatch, onInfo: setInfo, prediction: predictions[`${game.generation}:${active.id}`], onPrediction: value => setPredictions(previous => ({ ...previous, [`${game.generation}:${active.id}`]: value })) }) : panel("Computer players are using their Energy", h("p", null, "Your Energy plan will appear when they finish."));
    }
    else if (game.phase === "generation.review")
        decision = h(ReviewScreen, { game, onContinue: continueReview, predictions });
