...
function BuildShop({ game, player, guidedMode, onChoose, onInfo }) {
    ...
    const scored = available.map(technology => {
        ...
        const timingScore = impact.timing === "Helps now" ? 4 : impact.timing === "Helps forecast" ? 2 : 0;
    ...
    return h("div", { className: "guided-subpanel redesigned-build-shop" },
        ...
        h("div", { className: "technology-card-grid" }, ...visible.map(({ technology: tech, legality, impact }) => {
            ...
            const reachable = legality.legal;
            ...
            const canAffordOther = player.resources.constructionMaterials.warehouse >= cost.constructionMaterials;
            ...
            return h("article", { key: tech.id, className: `technology-choice-card ${reachable ? "available" : "locked"}` },
                h("div", { className: "technology-choice-heading" },
                    ...
                    badge(impact.timing, impact.timing === "Helps now" ? "reliable" : "energy")
                ),
                h("p", { className: "technology-purpose" }, impact.headline),
                h("div", { className: "technology-final-cost" },
                    h("span", { className: canAffordOther ? "" : "missing" }, h("small", null, "Other Materials"), h("strong", null, cost.constructionMaterials)),
                    h("span", { className: canAffordCritical ? "" : "missing" }, h("small", null, "Critical Minerals"), h("strong", null, cost.criticalMaterials)),
                    h("span", { className: player.knowledge >= cost.knowledgeRequired ? "" : "missing" }, h("small", null, "Readiness"), h("strong", null, `K${cost.knowledgeRequired}`))
                ),
                !legality.legal ? h(LockedReason, { reason: legality.reason }) : h("p", { className: "choice-benefit" }, impact.now),
                h("details", { className: "technology-details" },
                    ...
                    player.continentId === "europe" && (cost.constructionMaterials > 0 || cost.criticalMaterials > 0)
                        ? h("label", { className: "europe-recovery-choice" }, "Circular Recovery at Generation end", h("select", { value: europeRecoveryResource, onChange: event => setEuropeRecoveryResource(event.target.value) },
                            cost.constructionMaterials > 0 ? h("option", { value: "constructionMaterials" }, "Recover 1 spent Other Material") : null,
                            cost.criticalMaterials > 0 ? h("option", { value: "criticalMaterials" }, "Recover 1 spent Critical Material") : null))
                        : null
                ),
                h("div", { className: "technology-choice-actions" },
                    button("Details", () => onInfo(technologyExplanation(game, player, tech)), { kind: "ghost compact" }),
                    legality.legal ? button("Build", () => onChoose({ kind: "build", technologyId: tech.id, ...(player.continentId === "europe" ? { recoveryResource: europeRecoveryResource } : {}) }), { kind: "primary compact" }) : button("Locked", () => { }, { kind: "ghost compact", disabled: true, title: legality.reason })
                )
            );
        }))
    );
...
function GatherPanel({ game, player, onChoose }) {
    const choices = [
        ...
        { action: { kind: "extract", resource: "biomass" }, icon: "♣", title: "Biomass", note: "Renewable fuel for Biomass plants." }
    ];
    return h("div", { className: "clean-card-list" }, ...choices.map(choice => {
        ...
        return h("article", { key, className: `clean-choice-card ${legality.legal ? "available" : "locked"}` }, h("div", { className: "clean-choice-main" }, h("div", { className: "choice-icon" }, choice.icon), h("div", null, h("strong", null, choice.title), h("small", null, choice.note))), h("div", { className: "resource-preview" }, h("span", null, `This resource ${account.warehouse} · Warehouse total ${warehouseTotal(player)}/${game.config.rules.warehouseMaximum}`), h("span", null, `${account.currentContinent} left in your land`)), legality.legal ? h("p", { className: "choice-benefit" }, `After gathering: ${account.warehouse + amount} of this resource; ${warehouseTotal(player) + amount}/${game.config.rules.warehouseMaximum} total.`) : h(LockedReason, { reason: legality.reason }), button(legality.legal ? `Gather ${amount}` : "Unavailable", () => onChoose(choice.action), { kind: legality.legal ? "primary compact" : "ghost compact", disabled: !legality.legal, title: legality.reason }));
    }));
}
...
function DevelopmentControls({ game, player, onAction, onUndo, onReset, onTrade, tradeMessage, onInfo }) {
    ...
    const actionCards = [
        { id: "gather", icon: "▦", title: "Gather", text: "Normal extraction gives 1. Your regional specialty may give 2.", enabled: true },
    ...
    return h("div", { className: "action-panel clean-action-panel player-focus-panel" },
        ...
        !section ? h("div", { className: "secondary-game-actions" },
            button("⇄ Player Trade or World Market", () => setSection("trade"), { kind: "ghost", disabled: tradeLocked, title: tradeLocked ? "Trade is introduced in Generation 2 when Guided Help is on." : "Exchange Warehouse resources." }),
        ...
        section ? h("div", { className: "section-step" },
            ...
            section === "trade" ? h("div", { className: "trade-sections" }, h("h4", null, "World Market — no action · pay 2 for 1"), h(ImportChoices, { game, player, onChoose: choose }), h("h4", null, "Player Trade — initiator uses 1 action"), h(TradePanel, { game, player, onTrade, message: tradeMessage })) : null
        ) : null,
