...
function EnergyChain({ game, player, onInfo, planning = true, compact = false }) {
    ...
    const directNodes = Object.entries(chain.sourceByPathway)
        .filter(([, value]) => value > 0)
        .map(([pathway, value]) => {
            if (pathway === "fossil" && chain.fossilChain) {
                const fossilLoss = (chain.fossilChain.storageLoss ?? 0) + (chain.fossilChain.transformationLoss ?? 0);
                return h("span", { key: pathway, className: `flow-source source-${pathway}`, "aria-label": `${value} usable fossil Energy after ${fossilLoss} Energy loss` },
                    h("b", { "aria-hidden": "true" }, sourceIcons[pathway]),
                    h(FlowPips, { count: chain.fossilChain.grossEnergy }),
                    fossilLoss ? h("span", { className: "flow-mini-loss", "aria-hidden": "true" }, ...Array.from({ length: Math.min(4, fossilLoss) }, (_, index) => h("i", { key: index }))) : null,
                    h("b", { className: "flow-inline-icon", "aria-hidden": "true" }, "⚙"),
                    h(FlowPips, { count: value })
                );
            }
            if (pathway === "biomass") {
                return h("span", { key: pathway, className: `flow-source source-${pathway}`, "aria-label": `${value} usable Biomass Energy` },
                    h("b", { "aria-hidden": "true" }, sourceIcons[pathway]),
                    h("b", { className: "flow-inline-icon", "aria-hidden": "true" }, "⚙"),
                    h(FlowPips, { count: value })
                );
            }
            return h("span", { key: pathway, className: `flow-source source-${pathway}`, "aria-label": `${value} ${pathwayLabels[pathway]} Energy` },
                h("b", { "aria-hidden": "true" }, sourceIcons[pathway]),
                h(FlowPips, { count: value })
            );
        });
    const releasedNodes = Object.entries(chain.storageReleasedByPathway ?? {})
        .filter(([, value]) => value > 0)
        .map(([pathway, value]) => h("span", { key: pathway, className: `flow-source stored-source source-${pathway}`, "aria-label": `${value} previously stored ${pathwayLabels[pathway]} Energy released` },
            h("b", { "aria-hidden": "true" }, pathway === "hydro" ? "💧" : "▣"),
            h(FlowPips, { count: value })
        ));
    const pendingBattery = chain.chargedForNextGeneration ?? 0;
    const pendingReservoir = chain.reservoirCapturedForNextGeneration ?? 0;
    const summary = `${chain.directGeneration} current Energy plus ${chain.storageReleased} released from earlier storage; ${chain.deliveredEnergy} delivered and ${chain.lightProduced} Light produced.`;
    const mainInputs = [
        h("button", { type: "button", key: "direct", className: "graphic-flow-stage stage-source", onClick: () => onInfo({ eyebrow: "Energy flow", title: "Current sources", summary: `${chain.directGeneration} Energy`, details: [summary] }) },
            h("span", { className: "flow-stage-icon", "aria-hidden": "true" }, "⚡"),
            h("span", { className: "flow-stage-content source-cluster" }, ...(directNodes.length ? directNodes : [h("span", { key: "empty", className: "flow-empty" }, "0")]))
        )
    ];
    if (releasedNodes.length) {
        mainInputs.push(h("span", { key: "plus", className: "graphic-flow-plus", "aria-hidden": "true" }, "+"));
        mainInputs.push(h("button", { type: "button", key: "release", className: "graphic-flow-stage stage-storage-release", onClick: () => onInfo({ eyebrow: "Energy flow", title: "Released storage", summary: `${chain.storageReleased} Energy from an earlier Generation`, details: [summary] }) },
            h("span", { className: "flow-stage-icon", "aria-hidden": "true" }, "▣"),
            h("span", { className: "flow-stage-content source-cluster" }, ...releasedNodes)
        ));
    }
    return h("div", { className: `energy-chain graphic-energy-chain ${compact ? "compact" : ""}`, role: "group", "aria-label": summary },
        h("div", { className: "graphic-flow-row" },
            ...mainInputs,
            h("span", { className: "graphic-flow-arrow", "aria-hidden": "true" }, "›"),
            h("button", { type: "button", className: "graphic-flow-stage stage-grid", onClick: () => onInfo({ eyebrow: "Energy flow", title: "Grid", summary: `${chain.deliveredEnergy} of ${chain.gridCapacity} Energy delivered`, details: [summary] }) },
                h("span", { className: "flow-stage-icon", "aria-hidden": "true" }, "↔"),
                h("span", { className: "flow-stage-content" },
                    h(FlowPips, { count: chain.deliveredEnergy }),
                    chain.unusedEnergy ? h("span", { className: "flow-loss", "aria-label": `${chain.unusedEnergy} Energy unused` }, ...Array.from({ length: Math.min(8, chain.unusedEnergy) }, (_, index) => h("i", { key: index }))) : null
                )
            ),
            h("span", { className: "graphic-flow-arrow", "aria-hidden": "true" }, "›"),
            h("button", { type: "button", className: "graphic-flow-stage stage-lighting", onClick: () => onInfo({ eyebrow: "Energy flow", title: "Light", summary: `${chain.lightProduced} Light`, details: [summary] }) },
                h("span", { className: "flow-stage-icon", "aria-hidden": "true" }, "💡"),
                h("span", { className: "flow-stage-content" },
                    h(FlowPips, { count: chain.lightProduced, kind: "light" }),
                    chain.lightingLoss ? h("span", { className: "flow-loss", "aria-label": `${chain.lightingLoss} Energy lost in Lighting` }, ...Array.from({ length: Math.min(4, chain.lightingLoss) }, (_, index) => h("i", { key: index }))) : null
                )
            )
        ),
        ...
        compact ? null : button("Details", () => onInfo({
            ...
            summary,
            details: [
                `Current pathway Energy: ${chain.directGeneration}.`,
                ...
                `Fuel-storage loss: ${chain.fossilChain?.storageLoss ?? 0}.`,
