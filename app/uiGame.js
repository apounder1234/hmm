...
function MapResource({ game, player, resource, interactive, onExtract, expanded }) {
    ...
    return h("button", {
        ...
        onClick: event => { event.stopPropagation(); if (legality?.legal) onExtract(resource); },
        "aria-label": legality?.legal
            ? `Extract ${amount} ${resourceLabels[resource]} from ${getContinentProfile(game, player).name}`
            : `${resourceLabels[resource]} unavailable`
    }, content);
}
function ContinentMarker({ game, player, expanded, active, onToggle, onExtract, position }) {
    const continent = getContinentProfile(game, player);
    const condition = conditionDefinition(game, player);
    const interactive = active && game.phase === "generation.development" && player.controller.kind === "human";
    return h("article", {
        ...
        h("button", {
            ...
            h("span", { className: "marker-copy" }, h("strong", null, continent.name), expanded ? h("small", null, player.name) : null),
        ...
        h("div", { className: `map-resource-strip ${expanded ? "expanded" : "compact"}` },
            ...resourceKeys.map(resource => h(MapResource, { key: resource, game, player, resource, interactive, onExtract, expanded }))
        ),
        expanded ? h("div", { className: "continent-expanded-meta" },
            ...
            h("span", null, h("small", null, "Light"), h("strong", null, getLightingLevel(game, player.id)))
        ) : null
    );
}
...
function WorldArea({ game, expandedId, onToggle, onExtract, transfer }) {
    const positions = {
    ...
    return panel("World board", h("div", { className: "world-map-shell" },
        h("div", { className: `world-map ${expandedId ? "has-expanded-region" : ""}`, role: "group", "aria-label": "Six-continent game map", onClick: () => expandedId && onToggle(expandedId) },
            ...
            ...currentOrder(game).map(id => h(ContinentMarker, { key: id, game, player: game.players[id], expanded: id === expandedId, active: id === activeId, onToggle, onExtract, position: positions[game.players[id].continentId] })),
...
function EnergyChain({ game, player, onInfo, planning = true, compact = false }) {
    ...
    const afterStorage = Math.max(0, chain.generated - chain.storageLoss);
    const afterTransformation = Math.max(0, afterStorage - chain.transformationLoss);
    const installedTechnologies = player.installed.map(instance => getTechnology(game, instance.technologyId));
    const hasBattery = installedTechnologies.some(technology => technology.stage === "storage" && technology.storage?.type === "battery");
    const hasFossil = installedTechnologies.some(technology => technology.pathway === "fossil") || chain.sourceByPathway.fossil > 0;
    const hasBiomass = installedTechnologies.some(technology => technology.pathway === "biomass") || chain.sourceByPathway.biomass > 0;
    const stages = [
        { id: "source", label: "Source", value: `${chain.generated}`, unit: "Energy", detail: Object.entries(chain.sourceByPathway).filter(([, value]) => value > 0).map(([path, value]) => `${pathwayLabels[path]} ${value}`).join(" · ") || "No active source" }
    ];
    if (hasBattery || hasFossil || chain.storageLoss > 0)
        stages.push({ id: "storage", label: "Storage", value: `${afterStorage}`, unit: "Energy", detail: chain.storageLoss ? `−${chain.storageLoss}` : "No loss now" });
    if (hasFossil || hasBiomass || chain.transformationLoss > 0)
        stages.push({ id: "transformation", label: "Transformation", value: `${afterTransformation}`, unit: "Energy", detail: chain.transformationLoss ? `−${chain.transformationLoss}` : "No loss now" });
    stages.push(
        { id: "grid", label: "Grid", value: `${chain.deliveredEnergy}`, unit: "Energy", detail: `cap ${chain.gridCapacity}${chain.unusedEnergy ? ` · ${chain.unusedEnergy} unused` : ""}` },
        { id: "lighting", label: "Lighting", value: `${chain.lightProduced}`, unit: "Light", detail: chain.lightingLoss ? `−${chain.lightingLoss}` : "" }
    );
    const summary = `${chain.generated} Energy generated. ${chain.storageLoss} lost in storage. ${chain.transformationLoss} lost in transformation. ${chain.deliveredEnergy} delivered through the Grid. ${chain.lightProduced} Light possible.`;
    const openDetails = stage => onInfo({
        eyebrow: "Energy chain",
        title: stage.label,
        summary: stage.detail || `${stage.value} ${stage.unit}`,
        details: [summary, planning ? "These are current planning values." : "These values describe the resolved Generation."]
    });
    return h("div", { className: `energy-chain simple-energy-chain ${compact ? "compact" : ""}`, role: "group", "aria-label": summary },
        h("div", { className: "energy-chain-stages" },
            ...stages.flatMap((stage, index) => [
                h("button", { type: "button", key: stage.id, className: "energy-stage", onClick: () => openDetails(stage) },
                    h("small", null, stage.label),
                    h("strong", null, stage.value),
                    h("span", null, stage.unit),
                    stage.detail ? h("em", null, stage.detail) : null
                ),
                index < stages.length - 1 ? h("span", { key: `${stage.id}-arrow`, className: "chain-arrow", "aria-hidden": "true" }, "→") : null
            ].filter(Boolean))
        ),
        compact ? null : button("Details", () => onInfo({ eyebrow: "Energy chain", title: "Current system calculation", summary, details: [
            `Generated: ${chain.generated} Energy.`,
            `Storage loss: ${chain.storageLoss}.`,
            `Transformation loss: ${chain.transformationLoss}.`,
            `Grid delivered: ${chain.deliveredEnergy} of capacity ${chain.gridCapacity}.`,
            `Lighting produced: ${chain.lightProduced} Light.`,
            `Unused Energy: ${chain.unusedEnergy}.`
...
function PlayerBoard({ game, player, onInfo }) {
    ...
    return h("section", { className: "player-board compact-player-board" },
        ...
        h("div", { className: "player-board-core" },
            h("section", { className: "board-warehouse-section" },
                h("div", { className: "compact-section-heading" }, h("strong", null, `Warehouse ${warehouseTotal(player)}/${game.config.rules.warehouseMaximum}`), h("small", null, ability.name)),
                h(Warehouse, { game, player, onInfo })
            ),
            h("section", { className: "board-system-section" },
                h("div", { className: "compact-section-heading" }, h("strong", null, "Energy system"), h("small", null, `${player.installed.length} technologies installed`)),
                h(EnergyChain, { game, player, onInfo, compact: true })
            )
        ),
        h("div", { className: "compact-score-row" },
            stat("Light", player.cumulative.totalLight),
...
function DevelopmentControls({ game, player, onAction, onUndo, onReset, onTrade, tradeMessage, onInfo }) {
    ...
    return h("div", { className: "action-panel clean-action-panel development-board simplified-development-board" },
        ...
        h("section", { className: `compact-goal-strip status-${status.toLowerCase().replace(" ", "-")}` },
            ...
            h("span", null, h("small", null, "Resilience"), h("strong", null, `${resilience} Light`)),
            h("b", null, status)
        ),
        h("section", { className: "compact-system-panel" },
            h(EnergyChain, { game, player, onInfo }),
            h("div", { className: "compact-bottleneck-line" }, h("small", null, bottleneck.type === "none" ? "SYSTEM READY" : "CURRENT LIMIT"), h("strong", null, bottleneck.label), button("Why?", () => onInfo({ eyebrow: "Current system", title: bottleneck.label, summary: bottleneck.explanation, details: [constraint.explanation] }), { kind: "ghost compact" }))
        ),
        ...
        section ? h("section", { className: "section-step development-section" },
            ...
            section === "trade" ? h("div", { className: "trade-sections" },
                h("section", { className: "trade-mode-card" }, h("h4", null, "World Market"), h("p", null, "0 actions · pay 2 resources for 1"), h(ImportChoices, { game, player, onChoose: choose })),
...
function DispatchPanel({ game, player, onDispatch, onInfo, prediction, onPrediction }) {
    ...
    return h("div", { className: "action-panel dispatch-clean" },
        ...
        h("div", { className: "visual-flow educational-resolution separated-resolution", "aria-live": "polite" },
            ...[
                { id: "generated", label: "GENERATED", value: `${preview.grossEnergy} Energy`, detail: sourceRows.map(([path, value]) => `${pathwayLabels[path]} ${value}`).join(" · ") || "No generation" },
                ...(storageLoss > 0 || preview.stored > 0 || (preview.fuelConsumed?.fossilFuel ?? 0) > 0 ? [{ id: "storage", label: "STORAGE", value: `${Math.max(0, preview.grossEnergy - storageLoss)} remain`, detail: storageLoss ? `−${storageLoss} Energy` : "No loss" }] : []),
                ...(transformationLoss > 0 || (preview.fuelConsumed?.fossilFuel ?? 0) > 0 || (preview.fuelConsumed?.biomass ?? 0) > 0 ? [{ id: "transformation", label: "TRANSFORMATION", value: `${Math.max(0, preview.grossEnergy - storageLoss - transformationLoss)} remain`, detail: transformationLoss ? `−${transformationLoss} Energy` : "No loss" }] : []),
                { id: "grid", label: "GRID", value: `${deliveredEnergy} Energy`, detail: `Capacity ${preview.gridCapacity}${preview.curtailed ? ` · ${preview.curtailed} unused` : ""}` },
                { id: "lighting", label: "LIGHTING", value: `${preview.light} Light`, detail: lightingLoss ? `−${lightingLoss} Energy` : "", resultClass: preview.pointEarned ? "reliable" : preview.demandMet ? "demand-met" : "" }
            ].flatMap((stage, index, array) => [
                h("div", { key: stage.id, className: `flow-node ${stage.id === "lighting" ? `light-result ${stage.resultClass}` : ""}` }, h("small", null, stage.label), h("strong", null, stage.value), stage.detail ? h("span", null, stage.detail) : null),
                index < array.length - 1 ? h("div", { key: `${stage.id}-arrow`, className: "flow-arrow" }, "→") : null
            ].filter(Boolean))
        ),
    ...
    const body = game.phase === "game.complete"
        ...
        : h("main", { className: `game-main phase6 ${guidedMode ? "guided" : "strategy"} ${game.phase === "generation.development" ? "phase-development" : ""}` },
            h("div", { className: "game-left" },
                h(WorldArea, { game, expandedId, onToggle: toggleContinent, onExtract: extractFromMap, transfer: resourceTransfer }),
