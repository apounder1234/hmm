...
import { directTradeBlocked, getPlayer, log, resourceTypes, warehouseTotal } from "./rules.js";
// -----------------------------------------------------------------------------
...
export function executeDirectTrade(state, aId, bId, aGives, bGives) {
    ...
    if (directTradeBlocked(state))
        throw new Error("The active Global Event has shut down direct trade this Generation.");
    const activeId = state.turnOrder[state.activeTurnIndex];
    if (aId !== activeId)
        throw new Error("Only the active player may initiate a direct trade.");
    const initiator = getPlayer(state, aId);
    const recipient = getPlayer(state, bId);
    if (aId === bId)
        throw new Error("A player cannot trade with itself.");
    if (initiator.actionsRemaining <= 0)
        throw new Error(`${initiator.name} has no Development action available for trade.`);
    const maximum = state.config.trade.directTradeMaximumBundlePerSide ?? 2;
    const initiatorTotal = sum(aGives);
    const recipientTotal = sum(bGives);
    if (initiatorTotal < 1 || recipientTotal < 1 || initiatorTotal > maximum || recipientTotal > maximum)
        throw new Error(`Each side must exchange between 1 and ${maximum} resources.`);
    ...
    for (const resource of resourceTypes) {
        const offered = aGives[resource] ?? 0;
        const requested = bGives[resource] ?? 0;
        if (!Number.isInteger(offered) || !Number.isInteger(requested) || offered < 0 || requested < 0)
            throw new Error("Trade values must be non-negative integers.");
        if (initiator.resources[resource].warehouse < offered || recipient.resources[resource].warehouse < requested)
            throw new Error("Insufficient Warehouse resources for trade.");

    }
    const initiatorAfter = warehouseTotal(initiator) - initiatorTotal + recipientTotal;
    const recipientAfter = warehouseTotal(recipient) - recipientTotal + initiatorTotal;
    if (initiatorAfter > state.config.rules.warehouseMaximum || recipientAfter > state.config.rules.warehouseMaximum)
        throw new Error("Trade would exceed a total Warehouse capacity.");
    // Apply atomically after all legality checks. Calling this function represents explicit acceptance.
    for (const resource of resourceTypes) {
        const offered = aGives[resource] ?? 0;
        const requested = bGives[resource] ?? 0;
        initiator.resources[resource].warehouse = initiator.resources[resource].warehouse - offered + requested;
        recipient.resources[resource].warehouse = recipient.resources[resource].warehouse - requested + offered;
    }
    initiator.actionsRemaining--;
    initiator.completedTrades++;
    recipient.completedTrades++;
    initiator.currentMetrics.tradesCompleted++;
    recipient.currentMetrics.tradesCompleted++;
    for (const resource of resourceTypes) {
        initiator.currentMetrics.resourcesExported[resource] = (initiator.currentMetrics.resourcesExported[resource] ?? 0) + (aGives[resource] ?? 0);
        initiator.currentMetrics.resourcesImported[resource] = (initiator.currentMetrics.resourcesImported[resource] ?? 0) + (bGives[resource] ?? 0);
        recipient.currentMetrics.resourcesExported[resource] = (recipient.currentMetrics.resourcesExported[resource] ?? 0) + (bGives[resource] ?? 0);
        recipient.currentMetrics.resourcesImported[resource] = (recipient.currentMetrics.resourcesImported[resource] ?? 0) + (aGives[resource] ?? 0);
    }
    log(state, "trade.completed", `${initiator.name} and ${recipient.name} completed a direct trade. ${initiator.name} spent one Development action.`, initiator.id, { aId, bId, aGives: structuredClone(aGives), bGives: structuredClone(bGives), actionSpent: true });
...
function validateSummitBundle(state, proposer, recipient, proposerGives, recipientGives) {
    ...
    if (resourceTypes.some(r => (proposerGives[r] ?? 0) > 0 && (recipientGives[r] ?? 0) > 0))
        throw new Error("The same resource cannot move in both directions.");
    if (warehouseTotal(proposer) - sum(proposerGives) + sum(recipientGives) > state.config.rules.warehouseMaximum || warehouseTotal(recipient) - sum(recipientGives) + sum(proposerGives) > state.config.rules.warehouseMaximum)
        throw new Error("The Summit trade would exceed a total Warehouse capacity.");
