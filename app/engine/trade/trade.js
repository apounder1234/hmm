...
import { getKnowledgeRequirement } from "../continentRules.js";
...
export function prepareKnowledgeLink(state, borrowerId, lenderId, technologyId, paymentResource) {
    ...
    const knowledgeRequired = getKnowledgeRequirement(state, borrower, technology);
    const borrowerOwnKnowledge = borrower.knowledge + borrower.temporaryKnowledge;
    if (borrowerOwnKnowledge >= knowledgeRequired)
        throw new Error(`${borrower.name} already has enough Knowledge for ${technology.name}.`);
    if (lender.knowledge < knowledgeRequired)
        throw new Error(`${lender.name} needs permanent Knowledge ${knowledgeRequired} to help with ${technology.name}.`);
    ...
    borrower.assistanceKnowledge = Math.max(0, knowledgeRequired - borrowerOwnKnowledge);
    ...
    log(state, "knowledgeLink.used", `${lender.name} shared Knowledge ${knowledgeRequired} so ${borrower.name} could build ${technology.name}; ${lender.name} received 1 ${paymentResource}.`, borrower.id, { borrowerId, lenderId, technologyId, paymentResource, knowledgeRequired });
}
