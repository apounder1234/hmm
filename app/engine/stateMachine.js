import { performDevelopmentAction } from "./actions/actions.js";
...
import { executeDirectTrade, prepareKnowledgeLink } from "./trade/trade.js";
...
import { advanceWeather, setInitialCurrent, setInitialForecast } from "./weather/weather.js";
    ...
    switch (command.type) {
        ...
        case "revealPrepared": {
            if (state.phase !== "setup.revealPrepared")
                throw new Error("Prepared cards cannot be revealed now.");
            state.phase = "setup.rollForecast";
            for (const p of Object.values(state.players))
        ...
        case "knowledgeLinkBuild": {
            ...
            performDevelopmentAction(linked, command.borrowerId, { kind: "build", technologyId: command.technologyId });
...
export function currentPlayerId(state) { return state.phase === "generation.development" || state.phase === "generation.dispatch" ? activePlayerId(state) : null; }
