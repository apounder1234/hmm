...
function build(state, player, technologyId, options = {}) {
    ...
    const useDeferredBlueprint = options.allowPreparedPathway !== false && player.prepared.pathwayId && !player.prepared.pathwayUsed && player.prepared.foundingProjectDeferred && tech.pathway === player.prepared.pathwayId;
    const useFossilBlueprint = options.allowPreparedPathway !== false && tech.id === "combinedCycle" && player.prepared.fossilBlueprintAvailable;
    if (useDeferredBlueprint || useFossilBlueprint) {
        cm = Math.max(0, cm - 1);
...
export function foundingProjectDefinition(state, playerId) {
    ...
    return { kind: "technology", name: technology.name, technology, cost: technology.cost };
}
...
export function resolveFoundingProject(state, playerId, complete) {
    ...
    if (!canCompleteFoundingProject(state, playerId))
        throw new Error(`${player.name} needs ${project.cost.constructionMaterials} General Materials and ${project.cost.criticalMaterials} Critical Minerals for ${project.name}.`);
