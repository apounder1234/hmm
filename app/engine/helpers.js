...
export function emptyMetrics(target) { return { grossEnergy: emptyEnergy(), deliveredLight: 0, reliabilityTarget: target, reliabilityMet: false, systemLoss: { thermal: 0, battery: 0, lighting: 0, other: 0 }, curtailed: 0, storedEnd: 0, fuelConsumed: {}, resourcesExtracted: {}, tradesCompleted: 0, importsCompleted: 0, knowledgeGained: 0, technologiesBuilt: [] }; }
...
export function effectivePathwayOpportunity(state, player, pathway) {
    ...
    if (pathway !== "fossil")
        return opportunity;
    const account = player.resources.fossilFuel;
    const remaining = account.currentContinent + account.warehouse;
    if (remaining <= Math.floor(account.printedStarting / 2))
        opportunity -= 2;
    else if (remaining <= Math.floor(account.printedStarting * 3 / 4))
        opportunity -= 1;
