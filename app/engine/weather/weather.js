...
export function setInitialCurrent(state) {
    if (state.phase !== "setup.rollCurrent")
        throw new Error("Current Condition can only be rolled after Prepared selections.");
...
export function setInitialForecast(state) {
    if (state.phase !== "setup.rollForecast")
        throw new Error("Forecast can only be rolled after Prepared selections are revealed.");
