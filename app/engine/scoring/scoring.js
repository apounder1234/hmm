...
export function finalRanking(state) {
    ...
    rows.sort((a, b) => b.totalLight - a.totalLight || b.reliableGenerations - a.reliableGenerations || a.systemLoss - b.systemLoss || b.usableStoredEnergy - a.usableStoredEnergy || a.playerId.localeCompare(b.playerId));
    const result = [];
    let rank = 1;
    for (let i = 0; i < rows.length; i++) {
        const r = rows[i];
        const prev = rows[i - 1];
        if (i > 0 && prev && (r.totalLight !== prev.totalLight || r.reliableGenerations !== prev.reliableGenerations || r.systemLoss !== prev.systemLoss || r.usableStoredEnergy !== prev.usableStoredEnergy))
            rank = i + 1;
