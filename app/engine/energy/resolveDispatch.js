...
import { addEnergy, emptyEnergy, getPlayer, getTechnology, hasTechnology, log, pathways, totalEnergy } from "../helpers.js";
...
function captureOutput(state, player, pathway) {
    ...
    return Math.max(0, Math.min(ideal, base + localGenerationDelta(state, player, pathway)));
}
...
function operateFuelPlants(state, player, requests) {
    ...
    for (const i of player.installed) {
        ...
        if (requested < 1 || requested > t.maximumOutput)
            throw new Error(`${t.name} requested output outside 1-${t.maximumOutput}.`);
        const pathway = t.pathway;
        const remainingOpportunity = continent.opportunities[pathway] - usedByPathway[pathway];
...
function biomassRegrowth(state, player) {
    if (!hasTechnology(player, "managedForest"))
        return;
    let amount = 1;
    const c = conditionApplies(state, player);
    ...
    if (c?.effect.kind === "hydroDelta" && !hasTechnology(player, "basicReservoir") && !hasTechnology(player, "advancedReservoir") && c.effect.fallbackBiomassRegrowthDelta !== undefined)
...
export function resolveDispatch(state, playerId, plan) {
    ...
    player.reliabilityByGeneration[state.generation] = met;
    ...
    log(state, "dispatch.resolved", `${player.name} delivered ${light} Light with ${player.currentMetrics.systemLoss.thermal + player.currentMetrics.systemLoss.battery + player.currentMetrics.systemLoss.lighting} System Loss.`, player.id, { metrics: structuredClone(player.currentMetrics) });
}
