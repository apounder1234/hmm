...
import { featureEnabled } from "./modes.js";
// -----------------------------------------------------------------------------
...
export function emptyMetrics(target) { return { grossEnergy: emptyEnergy(), deliveredLight: 0, reliabilityTarget: target, reliabilityEligible: false, reliabilityMet: false, reliabilityPointEarned: false, reliabilityPointCapped: false, systemLoss: { thermal: 0, battery: 0, lighting: 0, other: 0 }, curtailed: 0, storedEnd: 0, storedPendingEnd: 0, energyFlow: { directByPathway: emptyEnergy(), storageReleasedByPathway: emptyEnergy(), chargedForNextGeneration: 0, reservoirCapturedForNextGeneration: 0 }, fuelConsumed: {}, resourcesExtracted: {}, tradesCompleted: 0, importsCompleted: 0, resourcesImported: {}, resourcesExported: {}, knowledgeGained: 0, appliedLearningGained: 0, appliedLearningSpent: 0, continentAbilityValue: 0, continentAbilityActivations: 0, continentPenaltyActivations: 0, biomassRegrown: 0, circularRecovery: null, fossilChain: null, technologiesBuilt: [], globalEventEffects: [] }; }
...
export function regionalExtractionYield(state, player, resource) {
    ...
    const rule = profile.resourceRule;
    if (!featureEnabled(state, "fullRegionalRules") || rule?.kind !== "extraction" || rule.resource !== resource)
        return normalYield;
    const uses = player.regionalExtractionUsesThisGeneration ?? 0;
    return uses < (rule.maximumUsesPerGeneration ?? 1) ? rule.yield : normalYield;
}
export function regionalExtractionSpecialtyAvailable(state, player, resource) {
    const profile = getContinentProfile(state, player);
    const rule = profile.resourceRule;
    return featureEnabled(state, "fullRegionalRules")
        && rule?.kind === "extraction"
        && rule.resource === resource
        && (player.regionalExtractionUsesThisGeneration ?? 0) < (rule.maximumUsesPerGeneration ?? 1);
}
export function gatherAmount(state, player, resource) {
    const account = player.resources[resource];
    if (!account)
        throw new Error(`Unknown resource ${resource}.`);
    let yieldAmount = regionalExtractionYield(state, player, resource);
    const event = activeGlobalEvent(state);
    if (event?.effect.kind === "extractionYieldCap" && event.effect.resource === resource)
        yieldAmount = Math.min(yieldAmount, event.effect.maximumYield);
...
// Global Event rules
// -----------------------------------------------------------------------------
export function getGlobalEvent(state, id) {
    const event = state.config.globalEvents?.find(item => item.id === id);
    if (!event)
        throw new Error(`Unknown Global Event ${id}.`);
    return event;
}
export function activeGlobalEvent(state) {
    const id = state.globalEvents?.activeDefinitionId ?? null;
    return id ? getGlobalEvent(state, id) : null;
}
export function globalEventDrawsThisGeneration(state) {
    return (state.config.globalEventRules?.drawGenerations ?? []).includes(state.generation);
}
export function worldMarketRate(state, player, resource) {
    const event = activeGlobalEvent(state);
    if (event?.effect.kind === "worldMarketRateOverride" && event.effect.resource === resource)
        return event.effect.rate;
    if (event?.effect.kind === "firstWorldMarketRate" && !player.globalEventUsage?.worldMarketUsed)
        return event.effect.rate;
    return state.config.trade.worldMarketExchangeRate;
}
export function worldMarketBlocked(state, resource) {
    const event = activeGlobalEvent(state);
    if (!event)
        return false;
    if (event.effect.kind === "tradeShutdown" && event.effect.worldMarketDisabled)
        return true;
    if (event.effect.kind === "worldMarketResourceBlocked" && event.effect.resource === resource)
        return true;
    if (event.effect.kind === "harvestFailure" && event.effect.blocksWorldMarket && event.effect.resource === resource)
        return true;
    return false;
}
export function directTradeBlocked(state) {
    const event = activeGlobalEvent(state);
    return Boolean(event?.effect.kind === "tradeShutdown" && event.effect.directTradeDisabled);
}
...
export function getEffectiveUpgradeCost(state, playerId, technologyOrId, options = {}) {
    ...
    if (globalEvent?.effect.kind === "firstBuildResourceDelta"
        && !player.globalEventUsage?.buildUsed
        && globalEvent.effect.resource === "constructionMaterials")
        addModifier(result, "constructionMaterials", globalEvent.effect.amount, globalEvent.name, "globalFirstBuild");
    if (featureEnabled(state, "fullRegionalRules")) {
...
export function getStartingTechnologyIds(state, continent) {
    void state;
    void continent;
    return ["basicGrid", "standardLighting"];
}
...
export function setOpeningWeather(state) {
    const openingPhases = ["setup.preparedSelection", "setup.summit", "setup.revealPrepared", "setup.foundingProjects", "setup.rollCurrent", "setup.rollForecast"];
    if (!openingPhases.includes(state.phase))
        throw new Error("Opening weather can only be rolled during setup.");
    // During setup there is only one public weather result: the forecast for Generation 1.
    // It becomes Current only after every Founding Project has been resolved.
    state.weather.current = null;
    state.weather.currentDie = null;
    state.weather.forecastDie = "A";
    const hadForecast = Boolean(state.weather.forecast);
    if (!hadForecast)
        state.weather.forecast = rollWeather(state);
    if (!hadForecast)
        log(state, "weather.openingForecast", `Generation 1 Forecast is ${state.weather.forecast}.`, null, { forecast: state.weather.forecast });
    return { current: null, forecast: state.weather.forecast };
}
export function activateOpeningWeather(state) {
    if (state.weather.current)
        return { current: state.weather.current, forecast: state.weather.forecast };
    if (!state.weather.forecast)
        setOpeningWeather(state);
    state.weather.current = state.weather.forecast;
    state.weather.currentDie = state.weather.forecastDie ?? "A";
    state.weather.forecastDie = state.weather.currentDie === "A" ? "B" : "A";
    state.weather.forecast = rollWeather(state);
    log(state, "weather.openingActivated", `Generation 1 Current Condition is ${state.weather.current}; Generation 2 Forecast is ${state.weather.forecast}.`, null, { current: state.weather.current, forecast: state.weather.forecast });
    return { current: state.weather.current, forecast: state.weather.forecast };
}
export function revealPreparedAndBeginFounding(state) {
    state.opening.revealed = true;
    state.phase = "setup.foundingProjects";
    state.opening.foundingIndex = Math.max(0, state.opening.foundingIndex ?? 0);
    for (const player of Object.values(state.players))
        log(state, "prepared.revealed", `${player.name}: ${player.prepared.pathwayId} / ${player.prepared.capabilityId}.`, player.id);
}
// Legacy setup commands remain load-compatible. New games reveal one setup forecast, then activate it after Founding Projects.
export function setInitialCurrent(state) {
    if (state.phase !== "setup.rollCurrent")
        throw new Error("Current Condition can only be rolled during legacy opening setup.");
    setOpeningWeather(state);
...
export function setInitialForecast(state) {
    if (state.phase !== "setup.rollForecast")
        throw new Error("Forecast can only be rolled during legacy opening setup.");
    setOpeningWeather(state);
    activateOpeningWeather(state);
    state.phase = "generation.start";
}
...
export function invariantErrors(state) {
    ...
    for (const p of Object.values(state.players)) {
        ...
        if (!Number.isInteger(p.regionalExtractionUsesThisGeneration ?? 0) || (p.regionalExtractionUsesThisGeneration ?? 0) < 0 || (p.regionalExtractionUsesThisGeneration ?? 0) > 1)
            errors.push(`${p.id}.regionalExtractionUsesThisGeneration invalid`);
        ...
        for (const i of p.installed) {
            ...
            const pending = i.pendingStorageInput ?? emptyEnergy();
            if (t.storage && totalEnergy(i.storageInput) + totalEnergy(pending) > t.storage.capacity)
                errors.push(`${p.id}.${i.instanceId} exceeds storage capacity`);
            if (pathways.some(pathway => !Number.isInteger(pending[pathway] ?? 0) || (pending[pathway] ?? 0) < 0))
                errors.push(`${p.id}.${i.instanceId}.pendingStorage invalid`);
        }
    }
    ...
    if (state.globalEvents?.activeDefinitionId && !state.config.globalEvents.some(event => event.id === state.globalEvents.activeDefinitionId))
        errors.push("Unknown active Global Event.");
    return errors;
}
