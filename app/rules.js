...
export function setOpeningWeather(state) {
    ...
    const hadCurrent = Boolean(state.weather.current);
    ...
    if (!hadCurrent || !hadForecast)
        log(state, "weather.opening", `Generation 1 Current Condition is ${state.weather.current}; Generation 2 Forecast is ${state.weather.forecast}.`, null, { current: state.weather.current, forecast: state.weather.forecast });
...
// Legacy setup commands remain load-compatible, but new games resolve both opening dice automatically.
