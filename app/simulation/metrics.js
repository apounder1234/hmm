...
export function extractPlayerResults(state, gameIndex) {
    ...
    return state.results.map(ranking => {
        ...
        return {
            ...
            rank: ranking.rank,
            winner: ranking.rank === 1,
            sharedWinner: ranking.rank === 1 && ranking.sharedRank,
            ...
            technicalAssistanceReceived: state.log.filter(event => event.type === "assistance.lent" && event.data?.receiverId === player.id).length,
