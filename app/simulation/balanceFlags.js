...
export function detectBalanceFlags(rows, byContinent, technologyPurchases, weatherRows, byStrategy = []) {
    ...
    if (fossilPressureRate < 0.2)
        flags.push(flag("fossil-pressure-rare", "warning", "Fossil depletion pressure is rarely reached", `Only ${(fossilPressureRate * 100).toFixed(1)}% of player-games used enough fuel to reduce effective Fossil Opportunity.`, fossilPressureRate, 0.2));
