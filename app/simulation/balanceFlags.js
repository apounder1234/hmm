...
export function detectBalanceFlags(rows, byContinent, technologyPurchases, weatherRows) {
    const flags = [];
    ...
    const assistance = rows.reduce((sum, row) => sum + row.technicalAssistanceReceived, 0);
    const assistanceRate = rows.length ? assistance / rows.length : 0;
    if (assistanceRate < 0.05)
        flags.push(flag("assistance-rare", "warning", "Technical Assistance is rarely used", `Technical Assistance averaged ${assistanceRate.toFixed(3)} uses per player-game.`, assistanceRate, 0.05));
    ...
    const depletedFossil = rows.filter(row => row.fossilFuelRemaining === 0).length;
    if (depletedFossil === 0)
        flags.push(flag("fossil-never-depletes", "warning", "Fossil reserves never deplete", "No simulated player exhausted its Fossil Fuel stock.", 0, 1));
